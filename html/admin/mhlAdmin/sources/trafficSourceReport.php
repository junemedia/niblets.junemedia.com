...
$sPageTitle = "MyHealthyLiving Sources - Traffic Source Report";
...
if (hasAccessRight($iMenuId) || isAdmin()) {
	...
	if (checkDate($monthFrom, $dayFrom, $yearFrom) && checkdate($monthTo, $dayTo,$yearTo)) {
		...
		$selectQuery = "SELECT srcName, sscName, count(*) counts
					FROM   source_tracking
					INNER JOIN source_codes ON source_tracking.stSource = source_codes.srcCode
					LEFT JOIN  sub_source_codes ON source_tracking.stSubSource = sub_source_codes.sscCode
					WHERE  date_format(stStamp,'%Y-%m-%d') >= '$dateFrom'
					AND    date_format(stStamp,'%Y-%m-%d') <= '$dateTo'	";
		if ($srcCode)
		{
			$selectQuery .= " AND stSource = '$srcCode'";
		}
		if ($subSrcCode)
		{
			$selectQuery .= " AND stSubSource = '$subSrcCode'";
		}
		
		$selectQuery .= " GROUP BY stSource, stSubSource
		...
		$sLogAddQuery = "INSERT INTO nibbles.trackNibbleUse(userName, pageName, dateTimeLogged, action) 
		  VALUES('$sTrackingUser', '$PHP_SELF', now(), \"View Traffic Source Report: $selectQuery\")"; 
		...
		if ($result) {
			if (mysql_num_rows($result) > 0) {
				...
				while ($row = mysql_fetch_object($result)) {
					...
					$reportData .= "<tr class=$bgcolorClass><td>$row->srcName</td>
	...
	$srcQuery = "SELECT *
				 FROM   source_codes
				 ORDER BY srcCode";
	$srcResult = mysql_query($srcQuery);
	while ($srcRow = mysql_fetch_object($srcResult)) {
	...
	$subSrcQuery = "SELECT *
				 ...
				 ORDER BY sscCode";
	$subSrcResult = mysql_query($subSrcQuery);
	while ($subSrcRow = mysql_fetch_object($subSrcResult)) {
