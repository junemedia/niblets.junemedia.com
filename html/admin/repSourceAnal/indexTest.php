	...
	if ($sExportExcel && $sExportEmails) {
		...
		if ($sViewReport != "Today's Report") {
					...
					$sReportQuery .= " count(distinct otDataHistory.email) AS uniqueUsers,
								count(otDataHistory.email) AS totalOffersTaken, sum(1 * otDataHistory.revPerLead) as totalRevenue";
					
					if (($sSourceCode != '' || $sSubSourceCode != '') && $sFilter == 'exactMatch') {
						$sReportQuery .= ",date_format(otDataHistory.dateTimeAdded,'%Y-%m-%d') AS dateAdded ";
					}
					$sReportQuery .= " FROM   otDataHistory
						 WHERE  date_format(otDataHistory.dateTimeAdded,\"%Y-%m-%d\") BETWEEN '$sTempDateFrom' AND '$sTempDateTo'";	
				...
				if ($sSourceCode != '') {
					...
					$sReportQuery .= " count(distinct otData.email) AS uniqueUsers,
								count(otData.email) AS totalOffersTaken, sum(1 * otData.revPerLead) as totalRevenue";
				...
				// if searched for src and sourceCode not in campaigns table, make empty array components
				...
				//echo count($aReportArray['sourceCode']);
