...
$sPageTitle = "Nibbles Lead Rejection Reason Codes - List/Delete Reason Codes";
...
if (hasAccessRight($iMenuId) || isAdmin()) {		
	if ($sDelete) {
		...
		$sDeleteQuery = "DELETE FROM reasonCodes
	 			   		WHERE  id = $iId"; 
	...
	while ($oRow = dbFetchObject($rSelectResult)) {
		...
		$sReasonCodeList .= "<tr class=$sBgcolorClass><TD>$oRow->code</td>
						<TD>$oRow->reason</td>
						<TD><a href='JavaScript:void(window.open(\"addReasonCode.php?iMenuId=$iMenuId&iId=".$oRow->id."\", \"reasonCodes\", \"height=450, width=600, scrollbars=yes, resizable=yes, status=yes\"));'>Edit</a>
	...
	$sAddButton ="<input type=button name=sAdd value=Add onClick='JavaScript:void(window.open(\"addReasonCode.php?iMenuId=$iMenuId\", \"reasonCodes\", \"height=450, width=600, scrollbars=yes, resizable=yes, status=yes\"));'>";
...
<tr><td class=header>Code</td>
<td class=header>Reason</td>
</tr>

<?php echo $sReasonCodeList;?>
<tr><td colspan=7 align=left><?php echo $sAddButton;?></td></tr>
</table>
