...
if (hasAccessRight($iMenuId) || isAdmin()) {
	...
	if ($sExportExcel && $sExportEmails) {
		...
		if ($sDateFrom && $sDateTo) {
			...
			$sTempDateTimeFrom = $sTempDateFrom." 00:00:00";
			$sTempDateTimeTo = $sTempDateTo." 23:59:59";
			...
			if ($sTempDateFrom == $sTempToday && $sTempDateTo == $sTempToday) {
				$sSourceCodeQuery = "SELECT distinct links.sourceCode
						 FROM   otData, links
						 WHERE  otData.sourceCode = links.sourceCode
						 AND	campaignTypeId = '4'
						 AND	dateTimeAdded between '$sTempDateTimeFrom' AND '$sTempDateTimeTo'
						 ORDER BY links.sourceCode"; 
			} else {
				$sSourceCodeQuery = "SELECT distinct links.sourceCode
			...
			if ($sViewReport != "Today's Report") {
				...
				while ($oSourceCodeRow = dbFetchObject($rSourceCodeResult)) {
					...
					$sReportQuery .= " count(distinct otDataHistory.email) AS uniqueUsers,
								...
								sum(otDataHistory.revPerLead * ".$fTempRate." ) as partnerNetRevenue";
					
					if (($sSourceCode != '' || $sSubSourceCode != '') && $sFilter == 'exactMatch') {
						$sReportQuery .= ",date_format(otDataHistory.dateTimeAdded,'%Y-%m-%d') AS dateAdded ";
					}
					$sReportQuery .= " FROM   otDataHistory, offers
						 WHERE  otDataHistory.offerCode = offers.offerCode
						 AND	otDataHistory.dateTimeAdded BETWEEN '$sTempDateTimeFrom' AND '$sTempDateTimeTo'
					...
					if ($sExcludeNonRevenue == 'Y') {
						$sReportQuery .= " AND isNonRevenue != '1' ";
					}
				...
				while ($oSourceCodeRow = dbFetchObject($rSourceCodeResult)) {
					...
					$sReportQuery .= " count(distinct otData.email) AS uniqueUsers,
								...
								sum(otData.revPerLead * ".$fTempRate." ) as partnerNetRevenue";
		...
		for ($i=0;$i< count($aReportArray['sourceCode']);$i++) {
			...
			if ( $aReportArray['totalOffersTaken'][$i] > 0) {
				...
				if ($sExportExcel) {
					...
					$aReportArray['clicks'][$i]."\t".$aReportArray['uniqueUsers'][$i]."\t".
					$aReportArray['conversion'][$i]."\t".$aReportArray['totalOffersTaken'][$i]."\t".
					$aReportArray['effectiveCpm'][$i]."\t".$aReportArray['ourNetRevenue'][$i].
					"\t".$aReportArray['partnerNetRevenue'][$i]."\t".$aReportArray['totalRevenue'][$i];
		...
		if ($sExportEmails) {
			...
			$sExportEmailQuery = ereg_replace("sum\(otData.revPerLead\) as totalRevenue","",$sExportEmailQuery);
		...
		if ($sExportExcel) {
			...
			$sFileName = "revShare_".$iCurrMonth.$iCurrDay."_".$iCurrHH.$iCurrMM.$iCurrSS.".xls";
			...
			$sFileName = "revShareEmails_".$iCurrMonth.$iCurrDay."_".$iCurrHH.$iCurrMM.$iCurrSS.".xls";
