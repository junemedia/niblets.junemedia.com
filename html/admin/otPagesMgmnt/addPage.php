...
$sPageTitle = "Nibbles Ot Pages Management - Add/Edit OT Page";
...
if ($sSaveClose || $sSaveNew || $sSaveContinue) {
	...
	if ($sPageName == '') {
		...
		$bKeepValues = true;
	} else if ( $iCheckRedirectTo && !remoteFileExists($sRedirectTo)) {
		$sMessage = "Redirect To If Offer Taken URL Is Not A Valid URL...";
		$bKeepValues = true;
	} else if ($iCheckRedirectToNotOfferTaken &&  $sRedirectToNotOfferTaken !='' && !remoteFileExists($sRedirectToNotOfferTaken)) {
		$sMessage = "Redirect To If Not Offer Taken URL Is Not A Valid URL...";
		$bKeepValues = true;
	} else {
		
		// set default require yes/no value if not selected and page is yes/no page
		if ($iDisplayYesNo && $sRequireYesNo == '') {
			$sRequireYesNo = "all";
		}
		...
		if (!($iId)) {
			...
			if (dbNumRows($rCheckResult) == 0) {
				...
				$sNoThankYouCode = addslashes($sNoThankYouCode);
				$sPage1ExtraText = addslashes($sPage1ExtraText);
				$sPage2ExtraText1 = addslashes($sPage2ExtraText1);
				$sPage2ExtraText2 = addslashes($sPage2ExtraText2);
				
				if ($sRequireGenderDob !='Y') { $sRequireGenderDob = 'N'; };

				
				// Insert record if everything is fine
				$sAddQuery = "INSERT INTO otPages(id, pageName, dateTimeAdded, title, notes, pageLayoutId, page2LayoutId, newPage2LayoutId, userFormLayoutId, newUserFormLayoutId, offerListLayoutId, 
							e1PageLayoutId, hiddenForm, minNoOfOffers, maxNoOfOffers, displayYesNo, requireYesNo, offerImageSize, offerFontSize,
							...
							passOnInboundQueryString, inboundVarMap, outboundVarMap, passOnPhpsessid, enableGoTo, isGoToPopUp, offersByPageMap, offersByCatMap, autoEmail, 
							defaultAutoRespId, isCobrand, isCobrandTemplate, displayPoweredBy, offerNotRequired, optOut, sureOptOut, sureOptOutText, noThankYouCode,
							pageBgColor, borderColor, fontColor, offerBgColor1, offerBgColor2, page2BgColor, displayPage2HeaderImage, page1ExtraText, page2ExtraText1, page2ExtraText2,
							...
							displayShoppingSpreeDisclaimerWithPPLink, showExitPopup, srcForExitPopup, requireGenderDob )
					 VALUES('$iNewId', '$sPageName', NOW(),\"$sTitle\", \"$sNotes\", '$iPageLayoutId', '$iPage2LayoutId', '$iNewPage2LayoutId', '$iUserFormLayoutId', '$iNewUserFormLayoutId', '$iOfferListLayoutId', 
							'$iE1PageLayoutId', '$iHiddenForm', '$iMinNoOfOffers', '$iMaxNoOfOffers', '$iDisplayYesNo', \"$sRequireYesNo\", '$sOfferImageSize', '$sOfferFontSize',
							...
							'$iPassOnPrepopCodes', '$iPassOnInboundQueryString', \"$sInboundVarMap\",  \"$sOutboundVarMap\", \"$iPassOnPhpsessid\", '$iEnableGoTo', '$iIsGoToPopUp', 
							'$iOffersByPageMap', '$iOffersByCatMap', '$iAutoEmail', \"$iDefaultAutoRespId\",
							'$iIsCobrand', '$iIsCobrandTemplate', '$iDisplayPoweredBy', '$iOfferNotRequired', '$iOptOut', '$iSureOptOut', \"$sSureOptOutText\", \"$sNoThankYouCode\", 
							'$sPageBgColor', '$sBorderColor', '$sFontColor', '$sOfferBgColor1', '$sOfferBgColor2', '$sPage2BgColor', '$iDisplayPage2HeaderImage',\"$sPage1ExtraText\", \"$sPage2ExtraText1\", \"$sPage2ExtraText2\",
							...
							'$iDisplayShoppingSpreeDisclaimer', '$iDisplayShoppingSpreeDisclaimerWithPPLink', '$sShowExitPopup', \"$sSrcForExitPopup\", \"$sRequireGenderDob\")";
				...
				$sLogAddQuery = "INSERT INTO nibbles.trackNibbleUse(userName, pageName, dateTimeLogged, action) 
				  VALUES('$sTrackingUser', '$PHP_SELF', now(), \"Add OT page: " . addslashes($sAddQuery) . "\")"; 
			...
			$sEditQuery = "UPDATE   otPages
					   SET 		title = \"$sTitle\",
								...
								newPage2LayoutId = '$iNewPage2LayoutId',
								userFormLayoutId = '$iUserFormLayoutId',
								newUserFormLayoutId = '$iNewUserFormLayoutId',
								offerListLayoutId = '$iOfferListLayoutId',
								e1PageLayoutId = '$iE1PageLayoutId',
								hiddenForm = '$iHiddenForm',
								minNoOfOffers = '$iMinNoOfOffers',
								maxNoOfOffers = '$iMaxNoOfOffers',
								displayYesNo = '$iDisplayYesNo', 
								requireYesNo = \"$sRequireYesNo\",
								...
								passOnPhpsessid = \"$iPassOnPhpsessid\",
								...
								defaultAutoRespId = '$iDefaultAutoRespId',
								isCobrand = '$iIsCobrand', 
								isCobrandTemplate = '$iIsCobrandTemplate', 
								displayPoweredBy = '$iDisplayPoweredBy', 								
								pageBgColor = '$sPageBgColor',
								offerNotRequired = '$iOfferNotRequired',
								optOut = '$iOptOut',
								...
								displayShoppingSpreeDisclaimerWithPPLink = '$iDisplayShoppingSpreeDisclaimerWithPPLink',
								showExitPopup = '$sShowExitPopup',
								srcForExitPopup = \"$sSrcForExitPopup\",
								requireGenderDob = \"$sRequireGenderDob\"
		 			   WHERE    id = '$iId'";
				...
				$sLogAddQuery = "INSERT INTO nibbles.trackNibbleUse(userName, pageName, dateTimeLogged, action) 
				  VALUES('$sTrackingUser', '$PHP_SELF', now(), \"Edit OT Page: " . addslashes($sEditQuery) . "\")"; 
				$rLogResult = dbQuery($sLogAddQuery); 
				echo  dbError(); 
				// end of track users' activity in nibbles		
	...
	if ($_FILES['image']['tmp_name'] && $_FILES['image']['tmp_name']!="none") {
		...
		$sNewImageFile  = "$sGblPageImagesPath/$sPageName/images/$sImageFileName";
	...
	if ($_FILES['e1HeaderImage']['tmp_name'] && $_FILES['e1HeaderImage']['tmp_name']!="none") {
		
		$sUploadedFileName = $_FILES['e1HeaderImage']['tmp_name'];
		
		//Get Extention
		$aImageFileNameArray = explode(".",$_FILES['e1HeaderImage']['name']);
		$i = count($aImageFileNameArray) - 1;
		$sImageFileExt = $aImageFileNameArray[$i];
		
		$sImageFileName = "e1_header_" . $iPageId . ".$sImageFileExt";
		$sNewImageFile  = "$sGblPageImagesPath/$sPageName/images/$sImageFileName";
		...
		// store the image file name in database
		$sUpdateQuery = "UPDATE otPages
						 SET     e1HeaderGraphicFile = '$sImageFileName'
						 WHERE  id = '$iPageId'";
		$rUpdateResult = dbQuery($sUpdateQuery);		
	}	
	...
	if ($sSaveContinue) {
		...
		if ($bKeepValues != true) {
			...
			$iDisplayShoppingSpreeDisclaimerWithPPLink = '';
			$sShowExitPopup = '';
			$sSrcForExitPopup = '';
		}
	}
...
if ($iId) {
	...
	if ($rResult) {
		...
		while ($oRow = dbFetchObject($rResult)) {
			...
			$sE1HeaderGraphicFile = $oRow->e1HeaderGraphicFile;
			
			if ($oRow->headerGraphicFile != '') {
				$sCurrentImage = "<img src='$sGblPageImagesUrl/$sPageName/images/$oRow->headerGraphicFile'>";
			} else {
			...
			$iNewPage2LayoutId = $oRow->newPage2LayoutId;
			$iUserFormLayoutId = $oRow->userFormLayoutId;
			$iNewUserFormLayoutId = $oRow->newUserFormLayoutId;
			$iOfferListLayoutId = $oRow->offerListLayoutId;
			$iE1PageLayoutId = $oRow->e1PageLayoutId;
			...
			$sRequireYesNo = $oRow->requireYesNo;
			...
			$iPassOnPhpsessid = $oRow->passOnPhpsessid;
			...
			$iDefaultAutoRespId = $oRow->defaultAutoRespId;
			$iIsCobrand = $oRow->isCobrand;
			$iIsCobrandTemplate = $oRow->isCobrandTemplate;
			$iDisplayPoweredBy = $oRow->displayPoweredBy;
			$iOfferNotRequired = $oRow->offerNotRequired;
			$iOptOut = $oRow->optOut;
			...
			$iDisplayShoppingSpreeDisclaimerWithPPLink = $oRow->displayShoppingSpreeDisclaimerWithPPLink;
			$sShowExitPopup = $oRow->showExitPopup;
			$sSrcForExitPopup = $oRow->srcForExitPopup;
			
			$sRequireGenderDob = $oRow->requireGenderDob;
...
// Prepare default auto responder options for selection box
$sAutoRespQuery = "SELECT *
				   FROM   pageAutoResponders
				   WHERE  pageId = '$iId'";

$rAutoRespResult = dbQuery($sAutoRespQuery);
$sAutoRespOptions .= "<option value=''>None";
while ($oAutoRespRow = dbFetchObject($rAutoRespResult)) {
	if ($oAutoRespRow->id == $iDefaultAutoRespId) {
		$sSelected = "selected";
	} else {
		$sSelected = "";
	}
	$sAutoRespOptions .= "<option value=$oAutoRespRow->id $sSelected>$oAutoRespRow->emailSub";
}
...
// if isCobrand template checked
$sIsCobrandTemplateChecked = "";
if ($iIsCobrandTemplate) {
	$sIsCobrandTemplateChecked = "checked";
}


// if optOut checked
$sOfferNotRequiredChecked = "";
if ($iOfferNotRequired) {
	$sOfferNotRequiredChecked = "checked";
}
...
// if display shopping spree disclaimer checked
...
// if display shopping spree disclaimer checked
$sDisplayShoppingSpreeDisclaimerWithPPLinkChecked = "";
if ($iDisplayShoppingSpreeDisclaimerWithPPLink) {
	$sDisplayShoppingSpreeDisclaimerWithPPLinkChecked = "checked";
}
...
$sPassOnPhpsessidChecked = "";
if ($iPassOnPhpsessid) {
	$sPassOnPhpsessidChecked = "checked";
}
...
$sOneYesSelected = "";
$sNotSelected = "";
switch($sRequireYesNo) {	
	case 'none':
		$sNoneSelected = "selected";
		break;
	case 'anyOne':
		$sAnyOneSelected = "selected";
		break;
	case 'oneYes':
		$sOneYesSelected = "selected";
		break;		
	case 'all':
		$sAllSelected = "selected";
		break;
	default:
	$sNotSelected = "selected";
} 

$sRequireYesNoOptions = "<option value='' $sNotSelected>
						 <option value='all' $sAllSelected>All Offers Required To Be Checked
						 <option value='anyOne' $sAnyOneSelected>Any One Offer Needs To Be Checked - Either Yes Or No
					     <option value='none' $sNoneSelected>None Of The Offers Required To Be Checked
					     <option value='oneYes' $sOneYesSelected>One Offer Required To Be Checked As Yes";
	...
	if ($rImageDir) {
		...
		while (($sFile = readdir($rImageDir)) != false) {	
			if (!is_dir("$sGblOtPagesPath/$sPageName/images/$sFile")) {
				...
				$sOtherImagesList .=  "<a href='JavaScript:void(window.open(\"$sGblPageImagesUrl/$sPageName/images/$sFile\",\"\",\"\"));'>$sGblPageImagesUrl/$sPageName/images/$sFile</a> 
...
// get list of page2 images, if offer is selected to edit
if ($iId) {
	$rFileDir = opendir("$sGblPageImagesPath/$sPageName/headers");
	if ($rFileDir) {

		while (($sFile = readdir($rFileDir)) != false) {	
			if (!is_dir("$sGblPageImagesPath/$sPageName/headers/$sFile")) {
					
				$headerFilesList .=  "<a href='JavaScript:void(window.open(\"$sGblPageImagesUrl/$sPageName/headers/$sFile\",\"\",\"\"));'>$sGblPageImagesUrl/$sPageName/headers/$sFile</a> 
...
<tR><td class=message colspan=2>If the value of the fields marked with * changed, requires page to be regenerated.<BR></td></tr>
	...
	<tr><td>E1 Page Header Image</td>
		<td colspan=3><input type=file name='e1HeaderImage'></td>
	</tr>
		
	<tr><td>Current Images</td><td colspan=3><?php echo $sOtherImagesList;?></td></tr>
	...
	<tr><td>Require Gender/DOB On User Form</td>
		<td colspan=3><input type=checkbox name='sRequireGenderDob' value='Y' <?php if ($sRequireGenderDob == 'Y') { echo ' checked '; }?>></td>
	</tr>
	...
	<tr><td>New Page2 Layout *</td>
		<td colspan=3><select name='iNewPage2LayoutId'>
		<?php echo $sNewPage2LayoutOptions;?>
		</select></td>
	</tr>
	...
	<tr><td>New User Form Layout *</td>
		<td colspan=3><select name='iNewUserFormLayoutId'>
		<?php echo $sNewUserFormLayoutOptions;?>
		</select></td>
	</tr>
	<tr><td>Offerlist Layout *</td>
		<td colspan=3><select name='iOfferListLayoutId'>
		<?php echo $sOfferListLayoutOptions;?>
		</select> <a href='JavaScript:void(window.open("<?php echo $sGblAdminSiteRoot;?>/offerListLayouts/addLayout.php?iMenuId=139&sReturnTo=iOfferListLayoutId", "", "height=450, width=600, scrollbars=yes, resizable=yes, status=yes"));'>Add Offerlist Layout</a></td>
	</tr>
	<tr><td>E1 Page Layout *</td>
		<td colspan=3><select name='iE1PageLayoutId'>
		<?php echo $sE1PageLayoutOptions;?>
		</select> </td>
	</tr>
	...
	<tr><td>Require Yes/No</td>
		<td colspan=3><select name=sRequireYesNo>
			 <?php echo $sRequireYesNoOptions;?>
			 </select></td>
	</tr>
	...
	<tr><td>Outbound Variable Map</td>
		<td colspan=3><input type=text name='sOutboundVarMap' value='<?php echo $sOutboundVarMap;?>' size=40><BR>
			Specify comma separated inbound=outbound variable name pairs to map.<br />You can use the code "pnd"
			to prepopulate a field with a 10-digit phone number (numerical only, no dashes).</td>
	</tr>
	<tr><td>Pass On PHP sessionId</td>
		<td colspan=3><input type=checkbox name='iPassOnPhpsessid' value='1' <?php echo $sPassOnPhpsessidChecked;?>></td>
	</tr>
	...
	<tr><td>Default Auto Responder</td>
		<td colspan=3><select name='iDefaultAutoRespId'>
		<?php echo $sAutoRespOptions;?>
		</select> <a href='JavaScript:void(window.open("<?php echo $sGblAdminSiteRoot;?>/pageAutoRespMgmnt/addAutoResp.php?iMenuId=139&sReturnTo=iDefaultAutoRespId", "", "height=450, width=600, scrollbars=yes, resizable=yes, status=yes"));'>Add Auto Responder</a></td>
	</tr>
	...
	<tr><td>Is Cobrand Template</td>
		<td colspan=3><input type=checkbox name='iIsCobrandTemplate' value='1' <?php echo $sIsCobrandTemplateChecked;?>></td>
	</tr>
	...
	<tr><td>Offer Not Required</td>
		<td colspan=3><input type=checkbox name='iOfferNotRequired' value='1' <?php echo $sOfferNotRequiredChecked;?>></td>
	</tr>
	<tr><td>Opt Out</td>
	...
	<tr><td>Display Shopping Spree Disclaimer With PP Link*</td>
		<td colspan=3><input type=checkbox name='iDisplayShoppingSpreeDisclaimerWithPPLink' value='1' <?php echo $sDisplayShoppingSpreeDisclaimerWithPPLinkChecked;?>></td>
	</tr>
