	...
	if ( $iCheckRedirectTo && !remoteFileExists($sRedirectTo)) {
		...
		if (!($iId)) {
			...
			if (dbNumRows($rCheckResult) == 0) {
				...
				$sAddQuery = "INSERT INTO otPages(id, pageName, title, notes, pageLayoutId, page2LayoutId, userFormLayoutId, offerListLayoutId, e1PageLayoutId, hiddenForm, minNoOfOffers, maxNoOfOffers, displayYesNo, offerImageSize, offerFontSize,
							...
							passOnInboundQueryString, inboundVarMap, outboundVarMap, enableGoTo, isGoToPopUp, offersByPageMap, offersByCatMap, autoEmail, autoEmailSub,
					 ...
					 VALUES('$iNewId', '$sPageName', \"$sTitle\", \"$sNotes\", '$iPageLayoutId', '$iPage2LayoutId', '$iUserFormLayoutId', '$iOfferListLayoutId', '$iE1PageLayoutId', '$iHiddenForm', '$iMinNoOfOffers', '$iMaxNoOfOffers', '$iDisplayYesNo', '$sOfferImageSize', '$sOfferFontSize',
	...
	if ($rResult) {
		...
		while ($oRow = dbFetchObject($rResult)) {
			...
			if ($oRow->headerGraphicFile != '') {
				$sCurrentImage = "<img src='".$sGblPageImagesUrl."/p/$sPageName/images/$oRow->headerGraphicFile'>";
			} else {
	...
	if ($rImageDir) {
		...
		while (($sFile = readdir($rImageDir)) != false) {	
			if (!is_dir("$sGblOtPagesPath/$sPageName/images/$sFile")) {
				...
				$sOtherImagesList .=  "<a href='JavaScript:void(window.open(\"$sGblPageImagesPath/$sPageName/images/$sFile\",\"\",\"\"));'>$sGblPageImagesUrl/$sPageName/images/$sFile</a> 
	...
	if ($rFileDir) {
		...
		while (($sFile = readdir($rFileDir)) != false) {	
			if (!is_dir("$sGblPageImagesPath/$sPageName/headers/$sFile")) {
				...
				$headerFilesList .=  "<a href='JavaScript:void(window.open(\"$sGblPageImagesPath/$sPageName/headers/$sFile\",\"\",\"\"));'>$sGblPageImagesUrl/$sPageName/headers/$sFile</a> 
