	...
	if ( $iCheckRedirectTo &&  !remoteFileExists($sRedirectTo)) {
		...
		if (!($iId)) {
			...
			if (dbNumRows($rCheckResult) == 0) {
				...
				$sAddQuery = "INSERT INTO otPages(pageName, title, notes, pageLayoutId, page2LayoutId, userFormLayoutId, hiddenForm, minNoOfOffers, maxNoOfOffers, displayYesNo,
							displayList, listText, listIdToDisplay, listPrecheck, submitText, redirectTo, redirectToNotOfferTaken, passOnPrepopCodes,
							...
							showExitPopup )
					 VALUES('$sPageName', \"$sTitle\", \"$sNotes\", '$iPageLayoutId', '$iPage2LayoutId', '$iUserFormLayoutId', '$iHiddenForm', '$iMinNoOfOffers', '$iMaxNoOfOffers', '$iDisplayYesNo',
							'$iDisplayList', \"$sListText\", '$iListIdToDisplay', '$iListPrecheck', \"$sSubmitText\", '$sRedirectTo', \"$sRedirectToNotOfferTaken\",
							...
							'$iDisplayShoppingSpreeDisclaimer', '$iShowExitPopup')";
...
if ($iId) {
	...
	if ($rResult) {
		...
		while ($oRow = dbFetchObject($rResult)) {
			...
			$iShowExitPopup = $oRow->showExitPopup;
		}
...
<tR><td class=message colspan=2>If the value of the fields marked with * changed, requires page to be regenerated.<BR></td></tr>
	...
	<tr><td>Show Exit Popup *</td>
		<td colspan=3><input type=checkbox name='iShowExitPopup' value='1' <?php echo $sShowExitPopupChecked;?>>
		<BR>Make sure you have <head> tag in page1 template to enable this.
