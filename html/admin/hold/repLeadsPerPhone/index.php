...
$sPageTitle = "Leads Per Phone Report";
...
if (hasAccessRight($iMenuId) || isAdmin()) {
	...
	if (!($sCurrOrder)) {
		switch ($sOrderColumn) {
			...
			break;
			case "phoneNo" :
			$sCurrOrder = $sPhoneOrder;
			$sPhoneOrder = ($sPhoneOrder != "DESC" ? "DESC" : "ASC");
			break;
			case "subSourceCode" :
	...
	$sSortLink = $PHP_SELF."?iMenuId=$iMenuId&iYearFrom=$iYearFrom&iMonthFrom=$iMonthFrom&iDayFrom=$iDayFrom&iYearTo=$iYearTo&iMonthTo=$iMonthTo&iDayTo=$iDayTo
							&iDbMailId=$iDbMailId&sViewReport=$sViewReport&iRecPerPage=$iRecPerPage&sUserName=$sUserName&sPageName=$sPageName&iLeadsPerPhone=$iLeadsPerPhone&sSourceCode=$sSourceCode
	...
	if ($sViewReport != "") {
		if (checkdate($iMonthFrom, $iDayFrom, $iYearFrom) && checkdate($iMonthTo, $iDayTo, $iYearTo) && !$bDateRangeNotOk) {
			if ($sAllowReport == 'N') {
				...
				$sTempDelete = "TRUNCATE TABLE tempPhoneStats";
				...
				if ($sShowSubSourceCode) {
					$sGroupByFilter = "GROUP BY phoneNo, sourceCode, subSourceCode";
				} else {
					$sGroupByFilter = "GROUP BY phoneNo";
				}

				$sTempQuery = "INSERT INTO tempPhoneStats (phoneNo,count,sourceCode,subSourceCode)
						SELECT phoneNo, count(phoneNo) as count, sourceCode, subSourceCode
				...
				$sDelete = "DELETE FROM tempPhoneStats WHERE count < $iLeadsPerPhone";
				$rDeleteResult = dbQuery($sDelete);
				echo dbError();
				
				
				$sReportQuery = "SELECT * FROM tempPhoneStats ORDER BY $sOrderColumn $sCurrOrder";
				...
				$rReportResult = dbQuery($sReportQuery);
					...
					if ($sTempPhone) {
						...
						<td>State</td><td>Zip</td><td>DateTimeAdded</td>
					...
					while ($oReportRow = dbFetchObject($rReportResult)) {
							...
							if ($sShowSubSourceCode) {
								if ($oReportRow->phoneNo == $sTempPhone) {
									$sReportContent .= "<tr class=$sBgcolorClass>
										<td><a href='$sSortLink&sTempPhone=$oReportRow->phoneNo'>$oReportRow->phoneNo</a></td>
...
<tr><td>Minimum Leads Per Phone Number:</td><td>
<input type=text name=iLeadsPerPhone value='<?php echo $iLeadsPerPhone;?>' size=2>
</td></tr>
...
<table cellpadding=0 cellspacing=0 bgcolor=c9c9c9 width=80% align=center border=1 bordercolor=#000000>
	...
	<tr><?php echo $sDateSentHeader;?>
		<td class=header><a href="<?php echo $sSortLink;?>&sOrderColumn=phoneNo&sPhoneOrder=<?php echo $sPhoneOrder;?>" class=header>Phone</a></td>
...
<tr><td colspan=4 align=left><hr color=#000000></td></tr>	
	...
	<tr><td colspan=4>
		...
		Clicking on actual phone number link will display related otData/userData.<br>
		Today's data not available; only have data from prior day and earlier.<br>
		No more than 7 day range allowed.<br>
		Count:  Number of leads collected with this phone number.<br>
