...
if ($sViewReport ) {
	...
	if ( checkdate($iMonthFrom, $iDayFrom, $iYearFrom) && checkdate($iMonthTo, $iDayTo, $iYearTo)) {
		...
		$sRepQuery = "SELECT sourceCode, sum(attempts) as totalAttempts, sum(rejects) as totalRejects,
					  ...
					  FROM   eTrackingSum, otPages
					  WHERE  eTrackingSum.pageId = otPages.id
					  AND	 submitDate BETWEEN '$sDateFrom' AND '$sDateTo'";
		
		if ($sSourceCode != '') {
					if ($iExactMatch) {
						$sRepQuery .= " AND eTrackingSum.sourceCode ='$sSourceCode' ";
					} else {
						$sRepQuery .= " AND eTrackingSum.sourceCode LIKE '$sSourceCode%' ";
					}
				}
				
		$sRepQuery .= " GROUP BY eTrackingSum.pageId, sourceCode";
		...
		if ($sViewReport ) {
			...
			$sAddQuery = "INSERT INTO nibbles.trackNibbleUse(userName, pageName, dateTimeLogged, action) 
			  VALUES('$sTrackingUser', '$PHP_SELF', now(), \"View report: $sRepQuery\")"; 
