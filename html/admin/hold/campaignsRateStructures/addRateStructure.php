...
if (($sSaveClose || $sSaveNew) && !($iId)) {
	...
	$sCheckQuery = "SELECT *
					...
					WHERE  rateType = '$sRateType'";
	$rCheckResult = dbQuery($sCheckQuery);
	if (dbNumRows($rCheckResult)>0) {
		$sMessage = "Record already exists with this campaign rate structure...";
		$bKeepValues = true;
	} else {
		$sAddQuery = "INSERT INTO campaignRateStructure (rateType, description)
				 	VALUES('$sRateType', \"$sDescription\")";
	...
	if (dbNumRows($rCheckResult)>0) {
		...
		$sEditQuery = "UPDATE campaignRateStructure
...
<table cellpadding=5 cellspacing=0 bgcolor=c9c9c9 width=95% align=center>
		<tr><TD>Campaign Rate Type</td><td><input type=text name=sRateType value='<?php echo $sRateType;?>'></td></tr>
