...
$sPageTitle = "Nibbles Editorial Orphan Offers Report";
...
if (hasAccessRight($iMenuId) || isAdmin()) {
	...
	$selectQuery = "SELECT OC.companyName, O.*
				FROM  edOffers O LEFT JOIN edOfferCategoryRel OCR ON O.id = OCR.offerId, edOfferCompanies OC
				WHERE OCR.offerId IS NULL
				AND O.companyId = OC.id
				$filterPart ";
	...
	//echo $selectQuery. mysql_error();
	$numRecords = mysql_num_rows($result);// Specify Page no. settings
	if (!($recPerPage)) {
		$recPerPage = 10;
	}
	if (!($page)) {
		...
		if (mysql_num_rows($result) > 0) {
			...
			while ($row = mysql_fetch_object($result)) {
				...
				$reportData .= "<tr class=$bgcolorClass><td>$row->offerCode</td>
							...
							<td><a href='JavaScript:void(window.open(\"addOffer.php?iMenuId=$iMenuId&id=".$row->id."&offerCode=".$row->offerCode."&recPerPage=$recPerPage&filter=$filter&exactMatch=$exactMatch&exclude=$exclude&searchIn=$searchIn&backTo=orphan\", \"AddOffer\", \"height=450, width=600, scrollbars=yes, resizable=yes, status=yes\"));'>Edit</a>
							&nbsp;| <a href='$sortLink&id=".$row->id."&deactivate=deactivate' >Deactivate</a>
							</td></tr>";
			}
		} else {
...
<tr><td colspan=6 align=right class=header><input type=text name=recPerPage value='<?php echo $recPerPage;?>' size=2 onChange='funcRecPerPage(this);'> &nbsp;Records Per Page &nbsp; &nbsp; 
...
<td><a href='<?php echo $sortLink;?>&orderColumn=expirationDate&expirationDateOrder=<?php echo $expirationDateOrder;?>' class=header>Expiration Date</a></td></tr>
<td></td>
<?php echo $reportData;?>
<TR><TD colspan=6 align=right class=header><?php echo $firstPageLink;?> &nbsp; <?php echo $prevPageLink;?> &nbsp; <?php echo $nextPageLink;?> &nbsp; <?php echo $lastPageLink;?> &nbsp; <?php echo $currentPage;?></td></tr>	
