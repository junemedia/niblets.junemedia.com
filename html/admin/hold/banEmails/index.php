...
$sPageTitle = "Nibbles Banned Emails - List/Delete Banned Emails";
...
if (hasAccessRight($iMenuId) || isAdmin()) {	
	...
	if ($sDelete) {
		...
		$sDeleteQuery = "DELETE FROM bannedEmails
	 			   		WHERE  id = $iId"; 
	...
	while ($oRow = dbFetchObject($rSelectResult)) {
		...
		$sBannedEmailList .= "<tr class=$sBgcolorClass><TD>$oRow->email</td>		
						<TD><a href='JavaScript:void(window.open(\"addBanEmail.php?iMenuId=$iMenuId&iId=".$oRow->id."\", \"bannedEmail\", \"height=450, width=600, scrollbars=yes, resizable=yes, status=yes\"));'>Edit</a>
					    &nbsp;<a href='JavaScript:confirmDelete(this,".$oRow->id.");' >Delete</a>
						</td></tr>";
		
		if($sExportBanned){
			$sExportData .= $oRow->email."\r\n";
		}
	}
	...
	if(($sExportBanned) && ($sExportData != '')){
		...
		$sFileName = 'bannedEmails_'.date('Y').date('M').date('d').'_'.date('H').date('i').date('s').'.txt';
	...
	$sAddButton ="<input type=button name=sAdd value=Add onClick='JavaScript:void(window.open(\"addBanEmail.php?iMenuId=$iMenuId\", \"bannedEmail\", \"height=450, width=600, scrollbars=yes, resizable=yes, status=yes\"));'>";
	$sExportLink ="<a href='".$_SERVER['PHP_SELF'].'?'.$_SERVER['QUERY_STRING']."&sExportBanned=1'>Export Banned Emails</a>";	
...
<tr><td class=header>Banned Emails</td><td><?php echo $sExportLink;?></td>
</tr>
