...
Script to Display List/Delete Stickers and Slogans
...
$sPageTitle = "Nibbles Stickers And Slogans Management";
...
if (hasAccessRight($iMenuId) || isAdmin()) {
	...
	if ($delete) {
		...
		$deleteQuery = "DELETE FROM edStickersAndSlogans
					    WHERE  id = '$id'"; 
		...
		if (mysql_num_rows($result) > 0) {
			...
			while ($row = mysql_fetch_object($result)) {
				...
				$stickersList .= "<tr class=$bgcolorClass><td>$dispHeadline</td>
								...
								<td><a href='JavaScript:void(window.open(\"addSticker.php?iMenuId=$iMenuId&id=".$row->id."\", \"AddSticker\", \"height=450, width=600, scrollbars=yes, resizable=yes, status=yes\"));'>Edit</a>
	...
	$addButton = "<input type=button name=add value=Add onClick='JavaScript:void(window.open(\"addSticker.php?iMenuId=$iMenuId\", \"\", \"height=450, width=600, scrollbars=yes, resizable=yes, status=yes\"));'>";
	...
	$sortStickersLink = "<a href='JavaScript:void(window.open(\"sortStickers.php?iMenuId=$iMenuId\",\"\",\"scrollbars=yes, resizable=yes, status=yes\"));'>Sort Stickers And Slogans</a>";
	$myFreeStickersPageLink = "<a href='JavaScript:void(window.open(\"$sGblMyFreeSiteRoot/displayContent.php/content/stickers\",\"\"));'>MyFree Stickers And Slogans Page</a>";
...
<tr><td colspan=2><?php echo $addButton;?> &nbsp; <?php echo $sortStickersLink;?></td><td colspan=5 align=right><?php echo $myFreeStickersPageLink;?></td></tr>
...
<TR><TD colspan=8 align=right class=header><input type=text name=recPerPage value='<?php echo $recPerPage;?>' size=2 onChange='funcRecPerPage(this);'> &nbsp;Records Per Page &nbsp; &nbsp; 
