...
if (hasAccessRight($iMenuId) || isAdmin()) {
	...
	if ( $sProcessLeads || $sSendLeads || $sSendFormPostLeads) {
		...
		if ($sTestMode && $sTestProcessingEmailRecipients == '') {
			...
			/*if ($sDailyProcessing) {
				...
				$startTime = date("U");
				echo "daily processing:<br>".__LINE__."&nbsp; &nbsp; start time : ".$startTime;
				echo '.';
				flush();
				ob_flush();
				...
				$endTime = date("U");
				echo "<br> &nbsp; &nbsp; query: ".$sOtDataUpdateQuery."<br>";
			...
			if ($sProcessLeads) {
				...
				$sThresholdTotal = "SELECT count(*) as total FROM $sOtDataTable WHERE dateTimeAdded > DATE_ADD(CURRENT_DATE, INTERVAL -1 DAY)";
				...
				$sThresholdPV = "SELECT count(*) as pv FROM $sOtDataTable WHERE dateTimeAdded > DATE_ADD(CURRENT_DATE, INTERVAL -1 DAY)
				...
				//echo "process start time : "
				$startTime = date("U");
				//echo "custom processing script:<br>".__LINE__."&nbsp; &nbsp; start time : ".$startTime;
				...
				$aMutExQueryTimes = array();
						...
						while ($oOffersRow = dbFetchObject($rOffersResult)) {
							...
							$sTempLeadsQuery = str_replace("otDataHistoryWorking", $sOtDataTable,$sTempLeadsQuery);
							$sTempLeadsQuery = str_replace("userDataHistoryWorking", $sUserDataTable,$sTempLeadsQuery);
							...
							if ($sTempLeadsQuery != '') {
								...
								if ($sOtDataTable == 'otData') {
									...
									$sTempLeadsQuery = eregi_replace("AND postalVerified = 'V'","AND process_leads.userDataHistoryWorking.postalVerified = 'V'",$sTempLeadsQuery);
								}
							}
				...
				ob_flush();
						...
						if (! $rTempLeadsResult) {
							...
							if ($sErrorInSendingLeads == '' && $iTempLeadsGroupId == 0 && $sTestMode == '') {
								...
								//echo "INFO(Line ".__LINE__.")Hey, is this section being called? ";
								//mail("bbevis@amperemedia.com,spatel@amperemedia.com","That thing from process leads, line".__LINE__,"Query that isn't running: ".$sProcessStatusUpdateQuery);
							}
	...
	if ($sSendLeadCounts) {
		...
		//mail('bbevis@amperemedia.com', $sLeadsCountEmailSubject, $sCountsEmailContent, $sHeaders);
	...
	$sRealTimePostLink = "<a href='JavaScript:void(window.open(\"realTimePost.php?iMenuId=$iMenuId\",\"setBack\",\"width=550 height=300, scrollbars=yes, resizable=yes\"));'>Real Time Post / Email</a>";
...
<table cellpadding=3 cellspacing=0 bgcolor=c9c9c9 width=95% align=center>
	...
	<!--<tr><td><b>Step 1:</b> </td><td><input type=submit name=sDailyProcessing value='Daily Maintenance' disabled></td></tr>
	<tr><td colspan=2><BR></td></tr>-->
	<tr><td><b>Step A:</b> </td>
	...
	<tr><td><b>Step B:</b> Select Either Offer Or Lead Group<BR>
	...
	<tr><td><b>Step 1:</b> </td><td><input type=submit name=sProcessLeads value='Process Leads'> &nbsp; &nbsp; &nbsp; Use Table With Today's Leads<input type=checkbox name=sUseCurrentTable value='Y' <?php echo $sUseCurrentTableChecked;?>>
	...
	<tr><td><b>Step 2:</b> </td><td nowrap><input type=submit name=sSendLeads value='Send Leads Out'>		
		...
		&nbsp; Test Email Recipient(s)<input type=text name=sTestProcessingEmailRecipients value="<?php echo $sTestProcessingEmailRecipients;?>" size=50></td>
		</tr>
	<tr><td><b>Step 3:</b> </td><td>
		<input type=submit name=sSendFormPostLeads value='Send Form Post Leads Out'> <A href='JavaScript:void(window.open("processLeadsDesc.php?sItem=sendFormPostLeads", "", "height=350, width=450, scrollbars=auto, resizable=yes, status=no"));' class=header>?</a>
		</td></tr>
		
	<tr><td><b>Step 4:</b> </td><td><input type=submit name=sSendLeadCounts value='Send Lead Counts to Fred'>
	...
	It mark all the leads as test leads which are collected in Test mode and mark 3401 leads as rejected. <b>Daily Maintainence has been deactivated and moved from this script to the overnight data move.</b><br></td></tr>-->
	
	<tr><td colspan=2><b>Step 1: Process Leads:</b> Click 'Process Leads'.  This may take about 20 minutes to complete.  Ensure that the 'Process All Leads' radio button is checked in step 2, and 
	...
	<tr><td colspan=2><b>Step 2: Send Leads Out:</b>  Click 'Send Leads Out'. This may take about 20 minutes to complete.  When that is finished, you'll see a list of all offers that were sent.
	This will send leads out to our clients.
	<br></td></tr>
	
	<tr><td colspan=2><b>Step 3: Send Form Post Leads Out:</b>  Click 'Send Form Post Leads Out'.  This may take about 15 minutes to complete.  When that is finished, you'll see "Form Post Lead Sent".
	This will send form post leads.
	<br></td></tr>
	
	<tr><td colspan=2><b>Step 4: Send Leads Counts to Fred:</b>  Click 'Send Leads Counts to Fred'. This may take about 3 minutes to complete.  When that is finished, you'll see "Lead Counts Email Is Sent...".
