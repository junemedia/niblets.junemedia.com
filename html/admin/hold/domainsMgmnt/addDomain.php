...
if (hasAccessRight($iMenuId) || isAdmin()) {
	if (($sSaveClose || $sSaveNew) && !($id)) {
		if ($sDomainName !='') {
			...
			if (dbNumRows($rCheckResult) > 0) {
				...
				$sAddQuery = "INSERT INTO domains (domainName,description,randomDomain) 
	...
	if ($id != '') {
		$sDescription = '';
		$selectQuery = "SELECT * FROM   domains WHERE  id = '$id'";
