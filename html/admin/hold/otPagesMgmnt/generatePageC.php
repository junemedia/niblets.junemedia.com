...
$sOtPageSubmitFile = "/newOtPageSubmitC.php";
$sOtPage2SubmitFile = "/newOtPage2SubmitC.php";
$sE1PageSubmitFile = "/e1PageSubmit.php";
$sE2PageSubmitFile = "/e2PageSubmit.php";

session_start();

if (hasAccessRight($iMenuId) || isAdmin()) {
$sGblSiteRoot = "http://".$_SERVER['SERVER_NAME'];
$sPageTitle = "Nibbles OT Pages - Create OT Pages";


$sCheckCoBrandQuery = "SELECT * FROM otPages 
					WHERE pageName !='sop' AND id='$iId'
					AND isCobrandTemplate = '1'";
$rCheckCoBrandResult = dbQuery($sCheckCoBrandQuery);
$iNumRows = mysql_num_rows($rCheckCoBrandResult);
if ($iNumRows > 0) {
	$bIsCoBrandTemplate = true;
} else {
	$bIsCoBrandTemplate = false;
}



/***********************  Loop through ot pages to generate ot pages  *******************/

$sOtPageQuery = "SELECT *
				 FROM   otPages
				 WHERE  pageName != 'sop'";

// If not all pages to be generated
if (! $sRegenerateAll) {
	 $sOtPageQuery .= " AND  id = '$iId'";
}
...
$sLogAddQuery = "INSERT INTO nibbles.trackNibbleUse(userName, pageName, dateTimeLogged, action) 
  VALUES('$sTrackingUser', '$PHP_SELF', now(), \"Regenerate C pages: $sOtPageQuery\")"; 
...
while ($oOtPageRow = dbFetchObject($rOtPageResult)) {
	...
	$sE1HeaderGraphicFile = $oOtPageRow->e1HeaderGraphicFile;
	$sE1FooterGraphicFile = $oOtPageRow->e1FooterGraphicFile;
	$iPageLayoutId = $oOtPageRow->pageLayoutId;
	$iNewPage2LayoutId = $oOtPageRow->newPage2LayoutId;
	$iNewUserFormLayoutId = $oOtPageRow->newUserFormLayoutId;
	$iE1PageLayoutId = $oOtPageRow->e1PageLayoutId;
	...
	$sBorderColor = $oOtPageRow->borderColor;
	...
	$iDisplayShoppingSpreeDisclaimerWithPPLink = $oOtPageRow->displayShoppingSpreeDisclaimerWithPPLink;
	$sShowExitPopup = $oOtPageRow->showExitPopup;
	$sSrcForExitPopup = $oOtPageRow->srcForExitPopup;
...
$sShoppingSpreeDisclaimerWithPPLink = '';
...
/********************  End getting page layout  *****************/


$sOtPageContent = ereg_replace("\[PAGE1_EXTRA_TEXT\]", $sPage1ExtraText, $sOtPageContent);


/********** get the user form layout and set it as ot page user form within the ot page *******/

$sUserFormLayoutQuery = "SELECT *
				 FROM   userFormLayouts
				 WHERE  id = '$iNewUserFormLayoutId'";
...
$sOtPageContent = ereg_replace("\[STYLE_SHEET\]", "<LINK rel=\"stylesheet\" href=\"/pageStyles.css\" type=\"text/css\"><SCRIPT LANGUAGE=JavaScript SRC=\"../libs/ajax.js\" TYPE=\"text/javascript\"></script>", $sOtPageContent);

// replace NO_THANK_YOU_CODE tag
$sOtPageContent = ereg_replace("\[NO_THANK_YOU_CODE\]", $sNoThankYouCode, $sOtPageContent);


if ($bIsCoBrandTemplate == false) {
	...
	$sOtPageContent = ereg_replace("\[PAGE_BG_COLOR\]", "\"<?php echo \$sTempPageBgColor; ?>\"", $sOtPageContent);
}


// replace header graphic in page layout ( Only graphic file path )
$sOtPageContent = ereg_replace("\[HEADER_GRAPHIC_FILE\]", "$sPageName/images/$sHeaderGraphicFile", $sOtPageContent);



// replace header graphic in page layout ( graphic file path alongwith surrounding html tags )
	...
	if (\$cbId != '') {
						...
						while (\$oRow = dbFetchObject(\$rResult)) {
						  \$sCbHeader = \$oRow->cbHeader;
						  \$sTopText = \$oRow->topText;
						  
						}
						echo \"".addslashes("<img src=\"\$sPageName/headers/\$sCbHeader\">")."\";\n
						
					  \n} else if (\$hd != '') {\n
						
						if (file_exists(\"\$sGblOtPagesPath/$sPageName/headers/\$hd.php\")) {\n
						
							include(\"\$sPageName/headers/\$hd.php\");\n
						} else {
							echo \"".addslashes("<img src=\"$sPageName/images/$sHeaderGraphicFile\">")."\";\n}\n
						} else {
					  		echo \"".addslashes("<img src=\"$sPageName/images/$sHeaderGraphicFile\">")."\";\n\n
	...
	$sOtPageContent = ereg_replace("\[TOP_TEXT\]", addslashes("<?php echo \$sTopText; ?>"), $sOtPageContent);
	//} else {
...
// replace step0 image in the page
//$sGblDisplayPageImagesUrl
if ( file_exists("$sGblOtPagesPath/$sPageName/images/step0.png")) {
	$sOtPageContent = ereg_replace("\[STEP0_GRAPHIC_FILE\]", "$sPageName/images/step0.png", $sOtPageContent);
} else if (file_exists("$sGblOtPagesPath/$sPageName/images/step0.jpg")) {
	$sOtPageContent = ereg_replace("\[STEP0_GRAPHIC_FILE\]", "$sPageName/images/step0.jpg", $sOtPageContent);
} else {
	$sOtPageContent = ereg_replace("\[STEP0_GRAPHIC_FILE\]", "$sPageName/images/step0.gif", $sOtPageContent);
}

// replace step1 image in the page
if ( file_exists("$sGblOtPagesPath/$sPageName/images/step1.png")) {
	$sOtPageContent = ereg_replace("\[STEP1_GRAPHIC_FILE\]", "$sPageName/images/step1.png", $sOtPageContent);
} else if (file_exists("$sGblOtPagesPath/$sPageName/images/step1.jpg")) {
	$sOtPageContent = ereg_replace("\[STEP1_GRAPHIC_FILE\]", "$sPageName/images/step1.jpg", $sOtPageContent);
} else {
	$sOtPageContent = ereg_replace("\[STEP1_GRAPHIC_FILE\]", "$sPageName/images/step1.gif", $sOtPageContent);
}

// replace step2 image in the page
if ( file_exists("$sGblOtPagesPath/$sPageName/images/step2.png")) {
	$sOtPageContent = ereg_replace("\[STEP2_GRAPHIC_FILE\]", "$sPageName/images/step2.png", $sOtPageContent);
} else if (file_exists("$sGblOtPagesPath/$sPageName/images/step2.jpg")) {
	$sOtPageContent = ereg_replace("\[STEP2_GRAPHIC_FILE\]", "$sPageName/images/step2.jpg", $sOtPageContent);
} else {
	$sOtPageContent = ereg_replace("\[STEP2_GRAPHIC_FILE\]", "$sPageName/images/step2.gif", $sOtPageContent);
}

// replace step3 image in the page
if ( file_exists("$sGblOtPagesPath/$sPageName/images/step3.png")) {
	$sOtPageContent = ereg_replace("\[STEP3_GRAPHIC_FILE\]", "$sPageName/images/step3.png", $sOtPageContent);
} else if (file_exists("$sGblOtPagesPath/$sPageName/images/step3.jpg")) {
	$sOtPageContent = ereg_replace("\[STEP3_GRAPHIC_FILE\]", "$sPageName/images/step3.jpg", $sOtPageContent);
} else {
	$sOtPageContent = ereg_replace("\[STEP3_GRAPHIC_FILE\]", "$sPageName/images/step3.gif", $sOtPageContent);
}

// replace arrow image in the page
if ( file_exists("$sGblOtPagesPath/$sPageName/images/arrow.png")) {
	$sOtPageContent = ereg_replace("\[ARROW_GRAPHIC_FILE\]", "$sPageName/images/arrow.png", $sOtPageContent);
} else if (file_exists("$sGblOtPagesPath/$sPageName/images/arrow.jpg")) {
	$sOtPageContent = ereg_replace("\[ARROW_GRAPHIC_FILE\]", "$sPageName/images/arrow.jpg", $sOtPageContent);
} else {
	$sOtPageContent = ereg_replace("\[ARROW_GRAPHIC_FILE\]", "$sPageName/images/arrow.gif", $sOtPageContent);
}


/*******************************  replace List Text values  ********************************/
// get the join list content to display
if ($iDisplayJoinList) {
...
/********************  End replacing join list tags  *********************/  


if ($bIsCoBrandTemplate == false) {
	// replace border color
	$sOtPageContent = ereg_replace("\[BORDER_COLOR\]", $sBorderColor, $sOtPageContent);
} else {
	$sOtPageContent = ereg_replace("\[BORDER_COLOR\]", "<?php echo \$sBorderColor; ?>", $sOtPageContent);
}



/**************  Replace font color tags  **********************/
if ($bIsCoBrandTemplate == true) {
	$sOtPageContent = ereg_replace("\[START_FONT_COLOR\]", "<font color=<?php echo \$sFontColor; ?>>", $sOtPageContent);
...
/**********************  End replacing font color tags  *********************/



/*********************  Replace custom query string variables  ***********************/
// custom query string variables starts with "[customQstring" 
// e.g. [customQstringVarName]
...
/**********************  End replacing custom query string variables  *****************/
...
if (strstr($sOtPageContent, "[ASP_FILE]")) {
	...
	$sNewVarString = "<?php
					if (\$PMID !='') {
						...
						echo \"<img src = \\\"$sPageName/images/".$sHeaderGraphicFile."\\\" border=0>\";
					}\n?>";
	
	
	$sOtPageContent = str_replace($sVarString, $sNewVarString, $sOtPageContent);
}
/************************  End placing asp file output  ***************************/
...
/***************  Replace offers list tags with php variables  ******************/
...
/***************  End replacing offers list tags  **********************/


/**********************  Replace shopping spree disclaimer tag  ***********************/
...
/***********************  End replacing shopping spree disclaimer  **********************/


/**********************  Replace shopping spree disclaimer with pp link tag  ***********************/

if ($iDisplayShoppingSpreeDisclaimerWithPPLink) {
$sShoppingSpreeQuery = "SELECT *
				   FROM   otPageDefinitions
				   WHERE  definition = 'shoppingSpreeDisclaimerWithPPLink'";
$rShoppingSpreeResult = dbQuery($sShoppingSpreeQuery) ;
echo dbError();
while ($oShoppingSpreeRow = dbFetchObject($rShoppingSpreeResult)) {
	$sShoppingSpreeDisclaimerWithPPLink = $oShoppingSpreeRow->definedValue;	
}
}
$sOtPageContent = ereg_replace("\[SHOPPING_SPREE_DISCLAIMER_WITH_PP_LINK\]", $sShoppingSpreeDisclaimerWithPPLink, $sOtPageContent);

/***********************  End replacing shopping spree disclaimer with pp link  **********************/


/************* replace prepop codes and outboundquery sting if have to pass with any links within the page  ****************/
...
/****************  End replacing prepop codes and outboundquery string  ****************/
...
$sOtPageContent = ereg_replace("\[MESSAGE\]", addslashes("<?php echo stripslashes(\$sMessage); ?>"), $sOtPageContent);


/******************  precheck join list if join list should be prechecked  **************/
if ($iDisplayJoinList) {
if ($iListPrecheck) {
...
/*******************  End precheck join list  *********************/


/**************  Replace page1 javascript vars  ********************/
$sDefineJavaVars = "\r\n<script LANGUAGE=\"javascript\"><?php\n echo \$_SESSION['sSesPage1JavaScriptVars'];?></script>\r\n</head>";

$sOtPageContent = eregi_replace("</head>",$sDefineJavaVars,$sOtPageContent);
/************************** End replacing page1 javascript vars  ******************/
 
 
/*****************  place popup code if exit popup is to be displayed  ********************/
if ($sShowExitPopup != '' && $sShowExitPopup != 'never') {
	
		
	if ($sShowExitPopup =='always') {
	...
	} else if ($sShowExitPopup == 'onlyForSrc') {



		$sExitPopupScript = "\r\n<script LANGUAGE=\"javascript\">var f=1; \n</script>\r\n
			<script src=\"../libs/jsPopFuncs.js\"></script>\r\n
		...
		for(var i=0; i< document.forms.length;i++) {
						...
						for(j=0;j<document.forms[i].elements.length;j++) {
							var eleName = document.forms[i].elements[j].id;
							if ( eleName == 'sSourceCode') {
								var tempSrc = document.forms[i].sSourceCode.value;
							}
						}
					}</script></head>";
		
		$sExitPopupText = "<body onunload=\"showPopupOnlyForSrc(f, '$sSrcForExitPopup' )\" ";
	} else if ($sShowExitPopup == 'notForSrc') {
		...
		$sExitPopupText = "<body onunload=\"showPopupNotForSrc(f, '$sSrcForExitPopup')\" ";
	}
...
if (\$sSiteId != '') {\n
	...
	while (list(\$key,\$val) = each(\$aGblSites)) {\n
		if (\$key == \$sSiteId) {\n
			// if current site is not as siteId, send user to the site as per siteId\n
			
			if (\$sCurrSite != \$val) {\n
				\$sNewSite =  \"http://\".\$aGblSiteNames[\$key];\n
				\$sNewUrl = \$sNewSite.\$_SERVER['PHP_SELF'];\n
				if (\$_SERVER['QUERY_STRING'] !='') {\n
					\$sNewUrl .= \"?\".\$_SERVER['QUERY_STRING'];	\n				
				}\n
	...
	while (list(\$key,\$val) = each(\$aGblSites)) {\n
		if (\$val == \$_SERVER['SERVER_ADDR']) {\n
			\$sSiteId = \$key;\n
								   ...
								   \$sPageName = substr(\$sFileName,0,strlen(\$sFileName)-6);\n
									...
									if (\$cbId != '') {\n
										\$_SESSION[\"iSesCbId\"] = \$cbId;\n
																				
										\$sQuery = \"SELECT * FROM   coBrandDetails WHERE  id = '\$cbId'\";
										\$rResult = dbQuery(\$sQuery);
										while (\$oRow = dbFetchObject(\$rResult)) {
										  	\$sTempPageBgColor = \$oRow->pageBgColor;
		...
		fwrite($rOtPageFile, " \n include(\"\$sGblIncludePath/newOtPageOffersC.php\");?>\n");
...
/************************** End storing the generated page  **************************/
...
$sTemp = 'onSubmit="return funcBeforeSubmit();"';
$sPage2Template = str_replace($sTemp, '', $sPage2Template);

$sPage2Template = ereg_replace("\[ACTION\]", $sOtPage2SubmitFile, $sPage2Template);
...
if ($bIsCoBrandTemplate == true) {
	...
	$sPage2Template = ereg_replace("\[PAGE_BG_COLOR\]", "<?php echo \$sPage2BgColor; ?>", $sPage2Template);
} else {
...
// replace the style sheet tag
$sPage2Template = ereg_replace("\[STYLE_SHEET\]", "<LINK rel=\"stylesheet\" href=\"../page2Styles.css\" type=\"text/css\"><SCRIPT LANGUAGE=JavaScript SRC=\"../libs/ajax.js\" TYPE=\"text/javascript\"></script>", $sPage2Template);

// replace the message
$sPage2Template = ereg_replace("\[MESSAGE\]", addslashes("<?php echo stripslashes(\$sMessage);?>"), $sPage2Template);	
	
// replace user form with php variable, to display user form for phone No and in case of errors
$sPage2Template = ereg_replace("\[USER_FORM\]", addslashes("<?php echo \$sUserForm;?>"), $sPage2Template);
...
if ($bIsCoBrandTemplate == true) {
	$sPage2Template = ereg_replace("\[START_FONT_COLOR\]", "<font color='<?php echo \$sFontColor; ?>'>", $sPage2Template);
...
if ($sShowExitPopup != '' && $sShowExitPopup != 'never') {
	...
	if ($sShowExitPopup =='always') {
		...
		$sExitPopupText = "<body onunload=\"showPopupNofForSrc(f, '$sSrcForExitPopup')\" ";
	}
	
	$sPage2Template = eregi_replace("<body", $sExitPopupText, $sPage2Template);

	// /home/sites/popularliving/html/images/page2Submit.gif page2Skip.gif
	// findout submit button of page2 layout and place OnClick="f=0" within the tag
}

	$sSubmitReplace = "<input type='image' STYLE='cursor: pointer' src='../images/page2Submit.gif' alt='Submit' onclick='JavaScript:return funcBeforeSubmit();'>
	&nbsp;&nbsp;&nbsp;<input type='image' STYLE='cursor: pointer' src='../images/page2Skip.gif' alt='Skip' name=sSkip value='skip' onclick='JavaScript:aSkipOffers.checked=true;document.forms[0].submit();f=0;'>";
//	echo "Test1<!--$sSubmitReplace--><br><br>";
	$sPage2Template = eregi_replace("<input type=submit name=sSubmit value=\"Submit Your Request!\">", "$sSubmitReplace", $sPage2Template);
//	echo "Test2<!--$sPage2Template--><br><br>";
//	exit;

/*
$sPage2JavaScript .= "function skipOffer(sOfferCode) {
   for (i = 0; i < document.form1.length; i++) {
          if (document.form1.elements[i].value == 'sOfferCode') {
...
include(\"\$sGblIncludePath/newOtPageIncludeC.php\");\n
include(\"\$sGblIncludePath/newOtPage2OffersC.php\");\n
...
if ($bOtPagesDir) {
	...
	unlink($sPageName."_c_2.php");
	$rOtPage2File = fopen($sPageName."_c_2.php", "w");
...
$sE1LayoutQuery = "SELECT *
				 FROM   ePageLayouts
				 WHERE  id = '$iE1PageLayoutId'";
//echo $sE1LayoutQuery;
$rE1LayoutResult = dbQuery($sE1LayoutQuery) ;
echo dbError();
while ($oE1LayoutRow = dbFetchObject($rE1LayoutResult)) {
	$sE1PageContent = $oE1LayoutRow->content;
}

// replace header and footer graphic in e1 page layout

$sE1PageContent = ereg_replace("\[ACTION\]", "$sE1PageSubmitFile", $sE1PageContent);


$sE1PageContent = ereg_replace("\[HEADER_GRAPHIC_FILE\]", "$sPageName/images/$sE1HeaderGraphicFile", $sE1PageContent);

$sE1PageContent = ereg_replace("\[OUTBOUND_QUERY_STRING\]", addslashes("<?php echo \$sOutboundQueryString; ?>"), $sE1PageContent);


$sE1PageContent = "<?php\n
include(\"../includes/paths.php\");\n

// start session before including e1PageInclude.php\n
session_start();\n
\$aPathInfo = explode(\"/\",\$_SERVER['PHP_SELF']);\n
								   \$sFileName = \$aPathInfo[count(\$aPathInfo)-1];\n
								   \$sPageName = substr(\$sFileName,0,strlen(\$sFileName)-7);\n
									\$iPageId = getPageId(\$sPageName);\n
									if(!(isset(\$_SESSION[\"iSesPageId\"]))) {
										\$_SESSION[\"iSesPageId\"] = '';\n
									}\n
...
if ($bOtPagesDir) {
	...
	unlink($sPageName."_e1.php");
	$rE1PageFile = fopen($sPageName."_e1.php", "w");
	
	
	if ($rE1PageFile) {
		fwrite($rE1PageFile, $sE1PageContent);		
		fclose($rE1PageFile);
	}

}
} // end of if ($iE1PageLayoutId)

/**************************  End generating e1 page  ********************/
...
$sE2PageContent = '';
if ($iE1PageLayoutId) {
$sE2LayoutQuery = "SELECT *
				 FROM   ePageLayouts
				 WHERE  id = '$iE1PageLayoutId'";
//echo $sE1LayoutQuery;
$rE2LayoutResult = dbQuery($sE2LayoutQuery) ;
echo dbError();
while ($oE2LayoutRow = dbFetchObject($rE2LayoutResult)) {
	$sE2PageContent = $oE2LayoutRow->content;
}

// replace header and footer graphic in e2 page layout
$sE2PageContent = ereg_replace("\[ACTION\]", "$sE2PageSubmitFile", $sE2PageContent);
$sE2PageContent = ereg_replace("\[HEADER_GRAPHIC_FILE\]", "$sPageName/images/$sE1HeaderGraphicFile", $sE2PageContent);
$sE2PageContent = ereg_replace("\[OUTBOUND_QUERY_STRING\]", addslashes("<?php echo \$sOutboundQueryString; ?>"), $sE2PageContent);

$sE2PageContent = "<?php\n
include(\"../includes/paths.php\");\n

// start session before including e2PageInclude.php\n
session_start();\n
\$aPathInfo = explode(\"/\",\$_SERVER['PHP_SELF']);\n
								   \$sFileName = \$aPathInfo[count(\$aPathInfo)-1];\n
								   \$sPageName = substr(\$sFileName,0,strlen(\$sFileName)-7);\n
									...
									\$_SESSION[\"iSesPageId\"] = \$iPageId;\n
include(\"\$sGblIncludePath/e2PageInclude.php\");?>\n". $sE2PageContent."\n
 ";

$bOtPagesDir = chDir($sGblOtPagesPath);
if ($bOtPagesDir) {
	unlink($sPageName."_e2.php");
	$rE2PageFile = fopen($sPageName."_e2.php", "w");

	if ($rE2PageFile) {
		fwrite($rE2PageFile, $sE2PageContent);		
		fclose($rE2PageFile);
	}
	chmod($sPageName."_e2.php",0777);
}
} // end of if ($iE1PageLayoutId)

/**************************  End generating e2 page  ********************/
