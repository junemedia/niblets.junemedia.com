...
if (hasAccessRight($iMenuId) || isAdmin()) {
	...
	$sSortLink = $PHP_SELF."?iMenuId=$iMenuId&sFilter=$sFilter&iExactMatch=$iExactMatch&iRecPerPage=$iRecPerPage";
		...
		if (dbNumRows($rSelectResult) > 0) {
			...
			while ($oRow = dbFetchObject($rSelectResult)) {
				...
				$sCampaignList .= "<tr class=$sBgcolorClass>
						...
						<TD><a href='JavaScript:void(window.open(\"addCampaign.php?iMenuId=$iMenuId&iId=".$oRow->id."&sFilter=$sFilter&sAlpha=$sAlpha&sExactMatch=$sExactMatch&iRecPerPage=$iRecPerPage\", \"AddCampaign\", \"height=450, width=600, scrollbars=yes, resizable=yes, status=yes\"));'>Edit</a>
