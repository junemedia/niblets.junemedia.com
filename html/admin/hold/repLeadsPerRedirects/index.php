...
$sPageTitle = "Leads Per Redirects Report";
...
if (hasAccessRight($iMenuId) || isAdmin()) {
	...
	$sSortLink = $PHP_SELF."?iMenuId=$iMenuId&iYearFrom=$iYearFrom&iMonthFrom=$iMonthFrom&iDayFrom=$iDayFrom&iYearTo=$iYearTo&iMonthTo=$iMonthTo&iDayTo=$iDayTo
							&iDbMailId=$iDbMailId&sViewReport=$sViewReport&iRecPerPage=$iRecPerPage&sUserName=$sUserName&sPageName=$sPageName&iLeadsPerRedirect=$iLeadsPerRedirect&sSourceCode=$sSourceCode
	...
	if ($sViewReport != "") {
		if (checkdate($iMonthFrom, $iDayFrom, $iYearFrom) && checkdate($iMonthTo, $iDayTo, $iYearTo) && !$bDateRangeNotOk) {
			if ($sAllowReport == 'N') {
				...
				$sTempDelete = "TRUNCATE TABLE tempRedirectRepStats";
				...
				if ($sShowSubSourceCode) {
					$sGroupByFilter = "GROUP BY sourceCode, subSourceCode";
				} else {
					$sGroupByFilter = "GROUP BY sourceCode";
				}

				$sTempQuery = "INSERT INTO tempRedirectRepStats (dateTimeAdded,sourceCode,subSourceCode,count)
						SELECT dateTimeAdded, sourceCode, subSourceCode, count(sourceCode) as count
						FROM otDataHistory
				...
				$sDelete = "DELETE FROM tempRedirectRepStats WHERE count < '$iLeadsPerRedirect'";
				$rDeleteResult = dbQuery($sDelete);
				echo dbError();
				
				
				$sReportQuery = "SELECT * FROM tempRedirectRepStats ORDER BY $sOrderColumn $sCurrOrder";
				...
				$rReportResult = dbQuery($sReportQuery);
					...
					if ($sTempSourceCode) {
						if ($sShowSubSourceCode) {
							$sTempSubSourceCode = " AND subSourceCode = '$sTempSubSourceCode'";
						} else {
							$sTempSubSourceCode = "";
						}
						$sDataQuery = "SELECT *
						...
						$sTempEmail = substr($sTempEmail, 0, strlen($sTempEmail)-1);			
						$sDataQuery = "SELECT *
							FROM userDataHistory
	   					 	WHERE email IN ($sTempEmail)";
						$rDataResult = dbQuery($sDataQuery);
					...
					while ($oReportRow = dbFetchObject($rReportResult)) {
							...
							if ($sShowSubSourceCode) {
								if ($oReportRow->sourceCode == $sTempSourceCode) {
									$sReportContent .= "<tr class=$sBgcolorClass>
										<td>$oReportRow->dateTimeAdded</td>	
										<td><a href='$sSortLink&sTempSourceCode=$oReportRow->sourceCode&sTempSubSourceCode=$oReportRow->subSourceCode'>$oReportRow->sourceCode</a></td>
										<td>$oReportRow->subSourceCode</td>
										<td>$oReportRow->count</td></tr>$sTempContent";
								} else {
									$sReportContent .= "<tr class=$sBgcolorClass>
										<td>$oReportRow->dateTimeAdded</td>	
										<td><a href='$sSortLink&sTempSourceCode=$oReportRow->sourceCode&sTempSubSourceCode=$oReportRow->subSourceCode'>$oReportRow->sourceCode</a></td>
										<td>$oReportRow->subSourceCode</td>
										<td>$oReportRow->count</td></tr>";
								}
							} else {
								if ($oReportRow->sourceCode == $sTempSourceCode) {
									$sReportContent .= "<tr class=$sBgcolorClass>
...
<tr><td>Minimum Leads Per Source Code:</td><td>
<input type=text name=iLeadsPerRedirect value='<?php echo $iLeadsPerRedirect;?>' size=2>
</td></tr>
...
<table cellpadding=0 cellspacing=0 bgcolor=c9c9c9 width=80% align=center border=1 bordercolor=#000000>
	...
	<tr><?php echo $sDateSentHeader;?>
		<td class=header><a href="<?php echo $sSortLink;?>&sOrderColumn=dateTimeAdded&sDateTimeAddedOrder=<?php echo $sDateTimeAddedOrder;?>" class=header>Date</a></td>
...
<tr><td colspan=4 align=left><hr color=#000000></td></tr>	
	...
	<tr><td colspan=4>
		...
		Clicking on actual source code link will display related otData/userData.<br>
		Today's data not available; only have data from prior day and earlier.<br>
		No more than 7 day range allowed.<br>
		Count:  Number of leads collected with this source code (and subSourceCode if 'Show SubSourceCode' is checked).<br>
