...
if (hasAccessRight($iMenuId) || isAdmin()) {
	...
	if (($sViewReport || $sExport)&& checkdate($iMonthFrom, $iDayFrom, $iYearFrom) && checkdate($iMonthTo, $iDayTo, $iYearTo)) {
		...
		if ($sShowTop10 == 'byRev') {
			$i=0;
			$sPagesSelected = '';
			$sTopPagesQuery = "SELECT pageId, sum($sLeadsCountCol) AS offersTaken, sum($sLeadsCountCol * revPerLead) as revenue
			...
			if (count($aPageId) > 0) {
				...
				while ($oPagesRow = dbFetchObject($rPagesResult)) {
					...
					if ($iRow == 1) {
						...
						$sLeadsQuery = "SELECT sum($sLeadsCountCol) AS offersTaken, sum($sLeadsCountCol * revPerLead) as revenue
										FROM   offerLeadsCountSum, offers
										WHERE  offerLeadsCountSum.offerCode = offers.offerCode
