...
if (hasAccessRight($iMenuId) || isAdmin()) {
	...
	if ($sDelete) {
		...
		$sDeleteQuery = "DELETE FROM excludeTLDsDataSales
	 			   		WHERE  id = $iId"; 
	...
	while ($oRow = dbFetchObject($rSelectResult)) {
		...
		$sExcludedTLDsList .= "<tr class=$sBgcolorClass><TD>$oRow->TLDs</td><td>$oRow->userName</td>
						<TD><a href='JavaScript:void(window.open(\"addTLDs.php?iMenuId=$iMenuId&iId=".$oRow->id."\", \"AddAccount\", \"height=450, width=600, scrollbars=yes, resizable=yes, status=yes\"));'>Edit</a>
	...
	$sAddButton ="<input type=button name=sAdd value=Add onClick='JavaScript:void(window.open(\"addTLDs.php?iMenuId=$iMenuId\", \"\", \"height=450, width=600, scrollbars=yes, resizable=yes, status=yes\"));'>";
...
<tr><td class=header>Excluded TLDs From Data Sales</td>
<td class=header>Account Executive</td>
</tr>

<?php echo $sExcludedTLDsList;?>
<tr><td colspan=7 align=left><?php echo $sAddButton;?></td></tr>
</table>
