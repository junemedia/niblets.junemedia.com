...
if (hasAccessRight($iMenuId) || isAdmin()) {
	...
	//  IF A SECTION HAS "&& FALSE" IN IT, IT IS TO DISABLE THAT FEATURE FOR THIS PAGE.
	if ($sDelete && false) {
		// if record deleted
	...
	while ($oRow = dbFetchObject($rResult)) {
				...
				$sPageList .= "<tr class=$sBgcolorClass><td>$oRow->pageName</td>
					...
					<td nowrap><a href='JavaScript:void(window.open(\"offers.php?iMenuId=$iMenuId&iId=".$oRow->id."\",\"offers\",\"scrollbars=yes, resizable=yes, status=yes\"));'>Offers</a> 
...
<tr><td colspan=7><?php echo $sAddButton;?></td>
	<td><a href='JavaScript:void(window.open("generatePage.php?iMenuId=<?php echo $iMenuId;?>&sRegenerateAll=y","genePage","scrollbars=yes, resizable=yes, status=yes"));' class=header>Regenerate All</a></td>
</tr>
