...
if (hasAccessRight($iMenuId) || isAdmin()) {
	...
	if ($sDelete) {
		$sDeleteQuery = "DELETE FROM emailCapCreative WHERE  id = $iId";
	...
	$sSelectQuery = "SELECT * FROM emailCapCreative ORDER BY name ASC";
	...
	while ($oRow = dbFetchObject($rSelectResult)) {
		...
		$sList .= "<tr class=$sBgcolorClass><td>$oRow->name</td>
						<td>$sContent...</td>
						<td><a href='JavaScript:void(window.open(\"addEmailCap.php?iMenuId=$iMenuId&id=".$oRow->id."\", \"AddContent\", \"height=650, width=650, scrollbars=yes, resizable=yes, status=yes\"));'>Edit</a>
	...
	$sAddButton ="<input type=button name=sAdd value=Add onClick='JavaScript:void(window.open(\"addEmailCap.php?iMenuId=$iMenuId\", \"\", \"height=650, width=650, scrollbars=yes, resizable=yes, status=yes\"));'>";
...
<tr><td class=header>Email Capture Creative Name</td><td class=header>Description</td>
</tr>
<?php echo $sList;?>
<tr><td colspan=5 align=left><?php echo $sAddButton;?></td></tr>
</table>
