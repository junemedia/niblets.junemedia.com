...
if ($sViewReport != "Today's Report") {
	...
	if (checkdate($iMonthFrom, $iDayFrom, $iYearFrom) && checkdate($iMonthTo, $iDayTo, $iYearTo)) {
		...
		if ($sViewReport != 'History Report') {
			...
			$sReportQuery = "SELECT count(otData.id) AS leadsCount, pageName, sum(1 * offers.revPerLead) as leadsRevenue
			...
			if ($iPostalVerified) {
				$sReportQuery = "SELECT count(otDataHistory.id) AS leadsCount, pageName, sum(1 * offers.revPerLead) as leadsRevenue
