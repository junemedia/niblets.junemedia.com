		...
		if (trim($fCommissionRate) != '') {
			if (checkdate($iMonthFrom, $iDayFrom, $iYearFrom) && checkdate($iMonthTo, $iDayTo, $iYearTo)) {
				...
				$sReportQuery = "SELECT companyName, otDataHistory.sourceCode ,
						 ...
						 FROM 	otDataHistory, offers, links, partnerCompanies
				...
				$sReportQuery .= " GROUP BY sourceCode ORDER BY $sOrderColumn $sCurrOrder ";
				...
				while ($oRepRow = dbFetchObject($rRepResult)) {
					...
					$fTempCommission = sprintf("%10.2f",round($oRepRow->commission, 2));
			
					$sReportContent .= "<tr class=$sBgcolorClass><td>$oRepRow->companyName</td><td>$oRepRow->sourceCode</td>
									<td align=right>$oRepRow->leadsCount</td><td align=right>$fTempCommission</td></tr>";
					$sExpReportContent .= "$oRepRow->companyName\t$oRepRow->sourceCode\t$oRepRow->leadsCount\t$fTempCommission\t\n";
					$iTotalCount += $oRepRow->leadsCount;
					$fTotalCommission += $oRepRow->commission;
				}

				$fTotalCommission = sprintf("%10.2f",round($fTotalCommission, 2));
			
				$sExpReportContent = "Partner Name\tSource code\tLead Counts\tCommission\n".$sExpReportContent;
...
if ($sExportExcel) {
		...
		$sFileName = "bdCommission_".$iCurrMonth.$iCurrDay."_".$iCurrHH.$iCurrMM.$iCurrSS.".xls";
...
<tr><td>Date From</td><td><select name=iMonthFrom><?php echo $sMonthFromOptions;?>
	...
	&nbsp; &nbsp; <input type=checkbox name=sExportExcel value='Export To Excel' <?php echo $sExportChecked;?>> Export To Excel</td>
...
<table cellpadding=0 cellspacing=0 bgcolor=c9c9c9 width=80% align=center border=1 bordercolor=#000000>
	...
	<tr><td colspan=4 ><BR><b>Notes -</b>
						<BR><BR> Count reflects leads which are postal verified and successfully delivered.
