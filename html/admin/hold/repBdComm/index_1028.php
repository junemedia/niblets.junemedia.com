...
if ($sViewReport ) {
	...
	$sSortLink = $PHP_SELF."?iMenuId=$iMenuId&iYearFrom=$iYearFrom&iMonthFrom=$iMonthFrom&iDayFrom=$iDayFrom&iYearTo=$iYearTo&iMonthTo=$iMonthTo&iDayTo=$iDayTo&iRepDesignated=$iRepDesignated&sViewReport=View Report";

	//if ($sViewReport) {
	if (checkdate($iMonthFrom, $iDayFrom, $iYearFrom) && checkdate($iMonthTo, $iDayTo, $iYearTo)) {
			
		$sReportQuery = "SELECT otDataHistory.sourceCode , count( otDataHistory.email )   AS leadsCount, substring(otDataHistory.sourceCode,1,14) as tempSourceCode
						 ...
						 AND	date_format(dateTimeAdded,'%Y-%m-%d') >= '$sDateFrom' AND '$sDateTo'
						 AND 	sendStatus = 'S' 
		...
		while ($oRepRow = dbFetchObject($rRepResult)) {
			...
			$sReportContent .= "<tr class=$sBgcolorClass><td>$oRepRow->tempSourceCode</td>
									<td align=right>$oRepRow->leadsCount</td><td>&nbsp;</td></tr>";
			$sExportData .= "$oRepRow->tempSourceCode\t$oRepRow->leadsCount\n";
			$iTotalCount += $oRepRow->leadsCount;
		}
		
		$sExportData .= "\nTotal Leads Count\t$iTotalCount\n";
...
<table cellpadding=0 cellspacing=0 bgcolor=c9c9c9 width=80% align=center border=1 bordercolor=#000000>
	...
	<tr><td width=45%><a href='<?php echo $sSortLink;?>&sOrderColumn=sourceCode&sSourceCodeOrder=<?php echo $sSourceCodeOrder;?>' class=header>Source Code</a></td>
	...
	<tr><td class=header>Total Leads Count</td><td align=right class=header><?php echo $iTotalCount;?></td></tr>
	<tr><td colspan=3 align=left><hr color=#000000></td></tr>
	<tr><td colspan=3 ><BR><b>Notes -</b>
