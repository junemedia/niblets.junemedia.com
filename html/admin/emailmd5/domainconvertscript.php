...
function SaveToFile($plainTextFile){
    ...
    if(MD5_EXPORT_FILE){
        ...
        for($i = 0; $i < $pages; $i++){
            ...
            $rows = 0;
            foreach($temp_array as $domain){
                // Clean up the domain first
                $domain = str_replace("*", "", $domain);
                if(strpos($domain, ".") !== false){
                    
                    echo "==>Process domain [@$domain] ... ";
                    
                    //process this domain
                    $sql = "SELECT email FROM `campaignerContacts` where email like '%@$domain'";
                    //echo $sql . "<br>";
                    ...
                    echo "Found [" . mysql_num_rows($result) . "] rows for domain [@$domain]\r\n";  
                }
            }
        ...
        //$content = implode($emailSame, "\n\r");
            echo "Success, wrote [$rows] rows of email address to file ($filename)\n\r";
            fclose($handle);
        } else {
