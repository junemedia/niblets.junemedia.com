...
Script to Display category display Report of HandCraftersVillage site
...
$sPageTitle = "Handcrafters Village Category Display Report";
...
if (hasAccessRight($iMenuId) || isAdmin()) {
	...
	$countQuery .= " SELECT clickDate, counts, title
					 FROM   categories, categoryDisplayCounts 
					 WHERE  categories.id = categoryDisplayCounts.categoryId
	...
	$sAddQuery = "INSERT INTO nibbles.trackNibbleUse(userName, pageName, dateTimeLogged, action)
			  VALUES('$sTrackingUser', '$PHP_SELF', now(), \"View Report: $sTempQuery1\")";
	...
	while ($countRow = dbFetchObject($countResult)) {
		...
		$categoryCounts = $countRow->counts;		
		...
		$reportData .= "<tr class=$bgcolorClass><td>$clickDate</td><td>$title</td>
						<td>$categoryCounts</td></tr>";		
	}
	
	if (dbNumRows($countResult) >0) {
		$reportData .= "<tr><td colspan=2 align=right class=header>Page Total Count &nbsp; &nbsp; </td><td>$totalCategoryCounts</td></tr>
						<tr><td colspan=2 align=right class=header>Grand Total Count &nbsp; &nbsp; </td><td>$grandTotalCategoryCounts</td></tr>";
	}
	...
	$reportLinks = "<a href='proDisplayReport.php?iMenuId=$iMenuId&iParentMenuId=$iParentMenuId&sParentMenuFolder=$sParentMenuFolder'>Project Report</a>
