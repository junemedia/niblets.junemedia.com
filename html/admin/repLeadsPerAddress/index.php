...
$sPageTitle = "Leads Per Address Report";
...
if (hasAccessRight($iMenuId) || isAdmin()) {
	...
	$sSortLink = $PHP_SELF."?iMenuId=$iMenuId&iYearFrom=$iYearFrom&iMonthFrom=$iMonthFrom&iDayFrom=$iDayFrom&iYearTo=$iYearTo&iMonthTo=$iMonthTo&iDayTo=$iDayTo
							&iDbMailId=$iDbMailId&sViewReport=$sViewReport&iRecPerPage=$iRecPerPage&sUserName=$sUserName&sPageName=$sPageName&iLeadsPerAddress=$iLeadsPerAddress&sSourceCode=$sSourceCode
	...
	if ($sViewReport != "") {
		if (checkdate($iMonthFrom, $iDayFrom, $iYearFrom) && checkdate($iMonthTo, $iDayTo, $iYearTo) && !$bDateRangeNotOk) {
			if ($sAllowReport == 'N') {
				...
				$sTempDelete = "TRUNCATE TABLE tempAddressStats";
				...
				if ($sShowSubSourceCode) {
					$sGroupByFilter = "GROUP BY address, zip, sourceCode, subSourceCode";
				} else {
					$sGroupByFilter = "GROUP BY address, zip";
				}

				$sTempQuery = "INSERT INTO tempAddressStats (address,zip,count,sourceCode,subSourceCode)
						SELECT address, zip, count(otDataHistory.email) as count, sourceCode, subSourceCode
				...
				$nibblesReportWhere = "WHERE otDataHistory.dateTimeAdded BETWEEN '$sDateFrom 00:00:00' AND '$sDateTo 23:59:59'
   					 	AND otDataHistory.email=userDataHistory.email
   					 	$sSourceCodeFilter
   					 	$sSubSourceCodeFilter
   					 	$sGroupByFilter";
				...
				$sAddQuery = "INSERT INTO nibbles.trackNibbleUse(userName, pageName, dateTimeLogged, action) 
				  VALUES('$sTrackingUser', '$PHP_SELF', now(), \"View report: Where clause: $nibblesReportWhere\")"; 
				...
				$sDelete = "DELETE FROM tempAddressStats WHERE count < $iLeadsPerAddress";
				$rDeleteResult = dbQuery($sDelete);
				echo dbError();

				
				$sReportQuery = "SELECT * FROM tempAddressStats ORDER BY $sOrderColumn $sCurrOrder";
				...
				$rReportResult = dbQuery($sReportQuery);
					...
					while ($oReportRow = dbFetchObject($rReportResult)) {
							...
							if ($sShowSubSourceCode) {
								if ($oReportRow->address == $sTempAddress) {
									$sReportContent .= "<tr class=$sBgcolorClass>
										<td><a href='$sSortLink&sTempAddress=$oReportRow->address&sTempZip=$oReportRow->zip'>$oReportRow->address</a></td>
...
<tr><td>Minimum Leads Per Address:</td><td>
<input type=text name=iLeadsPerAddress value='<?php echo $iLeadsPerAddress;?>' size=2>
</td></tr>
...
<table cellpadding=0 cellspacing=0 bgcolor=c9c9c9 width=80% align=center border=1 bordercolor=#000000>
	...
	<tr><?php echo $sDateSentHeader;?>
		<td class=header><a href="<?php echo $sSortLink;?>&sOrderColumn=address&sAddressOrder=<?php echo $sAddressOrder;?>" class=header>Address</a></td>
...
<tr><td colspan=4 align=left><hr color=#000000></td></tr>	
	...
	<tr><td colspan=4>
		...
		Clicking on actual address link will display related otData/userData.<br>
		Today's data not available; only have data from prior day and earlier.<br>
		No more than 7 day range allowed.<br>
		Count:  Number of leads collected with this address.<br>
