...
$sPageTitle = "Leads Per IP Address Report";
...
if (hasAccessRight($iMenuId) || isAdmin()) {
	...
	$sSortLink = $PHP_SELF."?iMenuId=$iMenuId&iYearFrom=$iYearFrom&iMonthFrom=$iMonthFrom&iDayFrom=$iDayFrom&iYearTo=$iYearTo&iMonthTo=$iMonthTo&iDayTo=$iDayTo
							&iDbMailId=$iDbMailId&sViewReport=$sViewReport&iRecPerPage=$iRecPerPage&sUserName=$sUserName&sPageName=$sPageName&iLeadsPerIp=$iLeadsPerIp&sSourceCode=$sSourceCode
	...
	if ($sViewReport != "") {
		if (checkdate($iMonthFrom, $iDayFrom, $iYearFrom) && checkdate($iMonthTo, $iDayTo, $iYearTo) && !$bDateRangeNotOk) {
			if ($sAllowReport == 'N') {
				...
				$sTempDelete = "TRUNCATE TABLE tempLeadsPerIp";
				...
				if ($sShowSubSourceCode) {
					$sGroupByFilter = "GROUP BY remoteIp, sourceCode, subSourceCode";
				} else {
					$sGroupByFilter = "GROUP BY remoteIp";
				}

				$sTempQuery = "INSERT INTO tempLeadsPerIp (remoteIp,count,sourceCode,subSourceCode)
						SELECT remoteIp, count(remoteIp) as count, sourceCode, subSourceCode
						FROM otDataHistory
				...
				$sDelete = "DELETE FROM tempLeadsPerIp WHERE count < $iLeadsPerIp";
				$rDeleteResult = dbQuery($sDelete);
				echo dbError();
				
				
				$sReportQuery = "SELECT * FROM tempLeadsPerIp ORDER BY $sOrderColumn $sCurrOrder";
				...
				$rReportResult = dbQuery($sReportQuery);
					...
					while ($oReportRow = dbFetchObject($rReportResult)) {
							...
							if ($sShowSubSourceCode) {
								if ($oReportRow->remoteIp == $sTempIp) {
									$sReportContent .= "<tr class=$sBgcolorClass>
										<td><a href='$sSortLink&sTempIp=$oReportRow->remoteIp'>$oReportRow->remoteIp</a></td>
...
<tr><td>Minimum Leads Per IP Address:</td><td>
<input type=text name=iLeadsPerIp value='<?php echo $iLeadsPerIp;?>' size=2>
</td></tr>
...
<table cellpadding=0 cellspacing=0 bgcolor=c9c9c9 width=80% align=center border=1 bordercolor=#000000>
	...
	<tr><?php echo $sDateSentHeader;?>
		<td class=header><a href="<?php echo $sSortLink;?>&sOrderColumn=remoteIp&sRemoteIpOrder=<?php echo $sRemoteIpOrder;?>" class=header>IP Address</a></td>
...
<tr><td colspan=4 align=left><hr color=#000000></td></tr>	
	...
	<tr><td colspan=4>
		...
		Clicking on actual ip address link will display related otData/userData.<br>
		Today's data not available; only have data from prior day and earlier.<br>
		No more than 7 day range allowed.<br>
		Count:  Number of leads collected with this ip address.<br>
