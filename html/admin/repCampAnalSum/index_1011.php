...
if (checkdate($iMonthFrom, $iDayFrom, $iYearFrom) && checkdate($iMonthTo, $iDayTo, $iYearTo)) {
	...
	if ($sPartnerCode != '') {
		...
		$sUniqueUsersNonPvQuery = "SELECT count(distinct userDataHistory.email) AS uniqueUsersNonPv
							  FROM    userDataHistory, otDataHistory, campaigns
							  WHERE userDataHistory.email = otDataHistory.email
							  AND      otDataHistory.sourceCode = campaigns.sourceCode
	...
	if ($sPartnerCode != '') {		
		...
		$sUniqueUsersQuery = "SELECT count(distinct otData.email) AS uniqueUsers
    						  FROM    otData, campaigns, partnerCompanies
    						  WHERE   otData.sourceCode = campaigns.sourceCode
							  AND     campaigns.partnerId  = '$iPartnerId'    						 
							  AND     date_format(otData.dateTimeAdded,\"%Y-%m-%d\") BETWEEN '$sDateFrom' AND '$sDateTo'";
		} else {
			$sUniqueUsersQuery = "SELECT count(distinct userDataHistory.email) AS uniqueUsers
    						  FROM    userDataHistory, otDataHistory, campaigns
    						  WHERE userDataHistory.email = otDataHistory.email 
							  AND      otDataHistory.sourceCode = campaigns.sourceCode
							  AND      campaigns.partnerId = '$iPartnerId'
		...
		$sUniqueUsersQuery = "SELECT count(distinct otData.email) AS uniqueUsers
    						  FROM    otData
    						  WHERE   date_format(otData.dateTimeAdded,\"%Y-%m-%d\") BETWEEN '$sDateFrom' AND '$sDateTo'";
		} else {
		$sUniqueUsersQuery = "SELECT count(distinct userDataHistory.email) AS uniqueUsers
    						  FROM   userDataHistory, otDataHistory
    						  WHERE  userDataHistory.email = otDataHistory.email 
	...
	if ($sPartnerCode != '') {
		//if ($sViewReport == 'History Report') {
	$sGrossRevenueQuery = "SELECT (count(otDataHistory.email) * offers.revPerLead) AS grossRevenue
      					   FROM    userDataHistory, otDataHistory, offers, campaigns
      					   WHERE userDataHistory.email = otDataHistory.email
 						   AND      otDataHistory.sourceCode = campaigns.sourceCode
 						   AND      campaigns.partnerId = '$iPartnerId'
		...
		//if ($sViewReport == 'History Report') {
			$sGrossRevenueQuery = "SELECT   (count(otDataHistory.email) * offers.revPerLead) AS grossRevenue
      						   FROM    userDataHistory, otDataHistory, offers
      						   WHERE   userDataHistory.email = otDataHistory.email
      						   AND     otDataHistory.offerCode = offers.offerCode 
      						   AND     date_format(otDataHistory.dateTimeAdded,\"%Y-%m-%d\") BETWEEN '$sDateFrom' 
      						   AND '$sDateTo'";
	...
	$sGrossRevenueQuery .= " GROUP BY offers.id";	
	...
	if ($sPartnerCode != '') {
		if ($sViewReport != 'History Report') {
			$sTotalOffersTakenQuery = "SELECT 	offers.offerCode, (count(otData.email) * offers.revPerLead) AS grossRevenue,
												count(otData.email) AS totalOffersTakenByUsers
      								 FROM    otData, campaigns, offers
      								 WHERE   otData.sourceCode = campaigns.sourceCode
				...
				$sTotalOffersTakenQuery = "SELECT 	offers.offerCode, (count(otDataHistory.email) * offers.revPerLead) AS grossPvRevenue,
												count(otDataHistory.email) AS totalOffersTakenByPvUsers
      								 FROM    userDataHistory, otDataHistory, campaigns, offers
      								 WHERE userDataHistory.email = otDataHistory.email
									 AND       otDataHistory.offerCode = offers.offerCode 
	 							 	 AND      otDataHistory.sourceCode = campaigns.sourceCode
	 								 AND      campaigns.partnerId = '$iPartnerId'
		...
		/*$sTotalOffersTakenQuery = "SELECT count(otDataHistory.email) AS totalOffersTakenByPvUsers
      							 FROM     userDataHistory, otDataHistory
      							 WHERE 	  userDataHistory.email = otDataHistory.email
								 AND       postalVerified = 'V'
      							 AND      date_format(otDataHistory.dateTimeAdded,\"%Y-%m-%d\") BETWEEN '$sDateFrom'
	...
	$sTotalOffersTakenQuery .= " GROUP BY offers.id";
	...
	if ($sPartnerCode != '') {		
		...
		$sClickQuery = "SELECT count(bdRedirectsTracking.id) as clicks
    						  FROM    bdRedirectsTracking, campaigns
	...
	if ($sPartnerCode != '') {
		/*if ($sViewReport != 'History Report') {
			$sGrossPvRevenueQuery = "SELECT offers.offerCode, (count(otData.email) * offers.revPerLead) AS grossPvRevenue,
	        		  				 		count(otData.email) AS validatedOffersTakenByUsers
      								 FROM    userData, otData, offers, campaigns
		      						 WHERE userData.email = otData.email
 									 AND      otData.sourceCode = campaigns.sourceCode
 									 AND      campaigns.partnerId = '$iPartnerId'
      								 AND       otData.offerCode = offers.offerCode 
      								 AND     date_format(userData.dateTimeAdded,\"%Y-%m-%d\") BETWEEN '$sDateFrom'
      								 AND '$sDateTo'";
		} else {*/
		
			$sValidatedOffersQuery = "SELECT offers.offerCode, (count(otDataHistory.email) * offers.revPerLead) AS validatedOffersGrossPvRevenue,
	...
	$sValidatedOffersQuery .= " GROUP BY offers.id";
	...
	if ($iClicks) {	
		...
		/*else {
			$fConversionRate = "N/A";
			//$fConversionRate = $iTotalOffersTakenByUsers / $iClicks;
			//$fConversionRate = sprintf("%10.4f",round($fConversionRate, 4));
		}*/
...
<tr><td>Date From</td><td><select name=iMonthFrom><?php echo $sMonthFromOptions;?>
	...
	<tr><td>Source Code</td><td colspan=3><input type=checkbox name=sAllSourceCodes value='Y' onChange=funcAllSource();> All &nbsp; &nbsp; &nbsp; &nbsp;  OR &nbsp; &nbsp; <input type=text name=sSourceCode value='<?php echo $sSourceCode;?>'> &nbsp; Exact Match
	<tr><td>Partner Code</td><td colspan=3><input type=checkbox name=sAllPartnerCodes value='Y' onChange=funcAllPartner();> All &nbsp; &nbsp; &nbsp; &nbsp; OR &nbsp; &nbsp;  <input type=text name=sPartnerCode value='<?php echo $sPartnerCode;?>'> &nbsp; Exact Match
