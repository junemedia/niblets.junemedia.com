...
$sPageTitle = "OT Links Analysis Summary Report";
...
if ( checkdate($iMonthFrom, $iDayFrom, $iYearFrom) && checkdate($iMonthTo, $iDayTo, $iYearTo)) {
	if ($sAllowReport == 'N') {
		...
		$sAddQuery = "INSERT INTO nibbles.trackNibbleUse(userName, pageName, dateTimeLogged, action) 
				VALUES('$sTrackingUser', '$PHP_SELF', now(), \"View report: $sViewReport. Date Range from $sDateTimeFrom to $sDateTimeFrom. Source code: $sSourceCode.  Partner ID: $sPartnerCode\")"; 
		...
		if ($sPartnerCode != '') {
			...
			$sUniqueUsersNonPvQuery = "SELECT count(distinct otDataHistory.email) AS uniqueUsersNonPv
								  FROM    otDataHistory, links
								  WHERE    otDataHistory.sourceCode = links.sourceCode
								  AND      links.partnerId = '$iPartnerId'
		...
		if ($sSourceCode != '') {
			$sUniqueUsersNonPvQuery .= " AND      otDataHistory.sourceCode = '$sSourceCode'";
		}
		...
		if ($sPartnerCode != '') {
			if ($sViewReport != 'History Report') {
				$sUniqueUsersQuery = "SELECT count(distinct otData.email) AS uniqueUsers
	    						  FROM    otData, userData, links
	    						  WHERE   otData.email = userData.email
			 					  AND	  otData.sourceCode = links.sourceCode
								  AND     links.partnerId  = '$iPartnerId'    	
		...
		if ($sSourceCode != '') {
			...
			$sUniqueUsersQuery .= " AND  $sOtData.sourceCode = '$sSourceCode'";
		}
		...
		/**************** unique users with at least one validated offer taken *****************/
		if ($sViewReport == 'History Report') {
			if ($sPartnerCode != '') {		
				$sUniqueUsersQuery = "SELECT count(distinct otDataHistory.email) AS uniqueUsersWithValidOffer
		    						  FROM    otDataHistory, links
		    						  WHERE   otDataHistory.sourceCode = links.sourceCode
									  AND     links.partnerId = '$iPartnerId'
		    						  AND     otDataHistory.postalVerified = 'V'
			...
			if ($sSourceCode != '') {
				$sUniqueUsersQuery .= " AND  otDataHistory.sourceCode = '$sSourceCode'";
			}
			...
			while ($oUniqueUsersRow = dbFetchObject($rUniqueUsersResult)) {
				$iUniqueUsersWithValidOffer = $oUniqueUsersRow->uniqueUsersWithValidOffer;
			}
		...
		if ($sViewReport == 'History Report') {
			...
			if ($sSourceCode != '') {
				$sGrossRevenueQuery .= " AND  otDataHistory.sourceCode = '$sSourceCode'";
			}
			
			$sGrossRevenueQuery .= " GROUP BY otDataHistory.offerCode";	
		...
		if ($sPartnerCode != '') {
			if ($sViewReport != 'History Report') {
				$sTotalOffersTakenQuery = "SELECT 	otData.offerCode, (count(otData.email) * revPerLead) AS grossRevenue,
										 ...
										 AND	 otData.sourceCode = links.sourceCode									 
		 								 AND     links.partnerId = '$iPartnerId'									
	      								 AND     otData.dateTimeAdded BETWEEN '$sDateTimeFrom' AND '$sDateTimeTo'";
				if ($sSourceCode != '') {
					$sTotalOffersTakenQuery .= " AND otData.sourceCode = '$sSourceCode'";
				}
				$sTotalOffersTakenQuery .= " GROUP BY otData.offerCode";
			} else {
				$sTotalOffersTakenQuery = "SELECT 	otDataHistory.offerCode, (count(otDataHistory.email) * otDataHistory.revPerLead) AS grossPvRevenue,
										 ...
										 AND     otDataHistory.postalVerified = 'V'									 
	      								 AND     otDataHistory.dateTimeAdded BETWEEN '$sDateTimeFrom' AND '$sDateTimeTo'";
				if ($sSourceCode != '') {
					$sTotalOffersTakenQuery .= " AND otDataHistory.sourceCode = '$sSourceCode'";
				}
				$sTotalOffersTakenQuery .= " GROUP BY otDataHistory.offerCode";
			}
		} else {
			if ($sViewReport != 'History Report') {
		...
		if ($sPartnerCode != '') {		
			if ($sViewReport != 'History Report') {
				$sClickQuery = "SELECT count(bdRedirectsTracking.id) as clicks
	    						  FROM    links, bdRedirectsTracking
	    						  WHERE   bdRedirectsTracking.sourceCode = links.sourceCode
								  AND     links.partnerId  = '$iPartnerId'    						 
								  AND     clickDate BETWEEN '$sDateFrom' AND '$sDateTo'";
				if ($sSourceCode != '') {
					$sClickQuery .= " AND bdRedirectsTracking.sourceCode = '$sSourceCode'";
				}
			} else {
				$sClickQuery = "SELECT sum(bdRedirectsTrackingHistorySum.clicks) as clicks
	    						  FROM    bdRedirectsTrackingHistorySum, links
	    						  WHERE   bdRedirectsTrackingHistorySum.sourceCode = links.sourceCode
								  AND      links.partnerId = '$iPartnerId'
								  AND     clickDate BETWEEN '$sDateFrom' AND '$sDateTo'";
				if ($sSourceCode != '') {
					$sClickQuery .= " AND bdRedirectsTrackingHistorySum.sourceCode = '$sSourceCode'";
				}
			}
		} else {
			if ($sViewReport != 'History Report') {
		...
		if ($sViewReport == 'History Report') {
			if ($sPartnerCode != '') {
				$sValidatedOffersQuery = "SELECT otDataHistory.offerCode, (count(otDataHistory.email) * revPerLead) AS validatedOffersGrossPvRevenue,
			...
			if ($sSourceCode != '') {
				$sValidatedOffersQuery .= " AND otDataHistory.sourceCode = '$sSourceCode'";
			}
			$sValidatedOffersQuery .= " GROUP BY otDataHistory.offerCode";
		...
		if ($sViewReport != 'History Report') {
			...
			$sReportContent = "<table cellpadding=0 cellspacing=0 bgcolor=c9c9c9 width=80% align=center border=1 bordercolor=#000000>
							...
							<tr><td align=center class=bigHeader colspan=2><BR>OT Links Analysis Summary Report <BR>From $iMonthFrom-$iDayFrom-$iYearFrom to $iMonthTo-$iDayTo-$iYearTo<BR><BR><BR></td></tr>
			...
			$sReportContent = "<table cellpadding=0 cellspacing=0 bgcolor=c9c9c9 width=80% align=center border=1 bordercolor=#000000>
							...
							<tr><td class=header align=right>Unique Users With At Least One Valid Offer:</td><td> $iUniqueUsersWithValidOffer</td></tr>
							...
							<tr><td class=header align=right>Net Revenue:<BR>(Taking Offer Validation Into Account)</td><td> \$".$fValidatedOffersGrossPvRevenue."</td></tr>
							<tr><td colspan=2><BR></td></tr>";
		}
		...
		$sReportContent .= "<tr><td colspan=2 class=header><BR>Notes - </td></tr>
							...
							<BR>Gross Unique Users in Source Analysis Report may be higher than gross unique users in Links Analysis Report
						...
						Links Analysis Report<br>
								...
								<BR><BR>Total offers taken by Pv users is the count of offers taken by Pv users and may include dups, mutaully exclusive, not custom verified.
								<BR><br>Validated offers taken by Pv users is the count of valid Pv offers after any type of custom verification, dedup and mutually exclusive offers.								
								<BR><BR>Approximate time to run this report - $iScriptExecutionTime seconds
								</td></tr><tr><td colspan=2><BR><BR></td></tr>";
		if ($sShowQueries) {
...
<tr><td>Date From</td><td><select name=iMonthFrom><?php echo $sMonthFromOptions;?>
	...
	</td><td colspan=2><input type=checkbox name=sShowQueries value='Y' <?php echo $sShowQueriesChecked;?>> Show Queries</td></tr>
</table>
