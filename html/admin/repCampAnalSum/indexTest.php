...
if (checkdate($iMonthFrom, $iDayFrom, $iYearFrom) && checkdate($iMonthTo, $iDayTo, $iYearTo)) {
	...
	//echo "<BR>nuniq ".$iUniqueUsersNonPv;
	...
	if ($sPartnerCode != '') {		
		...
		$sUniqueUsersQuery = "SELECT count(distinct userDataHistory.email) AS uniqueUsers
    						  ...
    						  AND    postalVerified = 'V'
		 					  AND    verified != 'I'
							  AND    date_format(otDataHistory.dateTimeAdded,\"%Y-%m-%d\") BETWEEN '$sDateFrom' AND '$sDateTo'
							  GROUP BY userDataHistory.email";	
		}
	}
	if ($sSourceCode != '') {
	...
	/*if ($sViewReport != 'History Report') {
		$sUniqueUsersQuery .= " GROUP BY userDataHistory.email";
	}*/
	...
	if ($sPartnerCode != '') {
		//if ($sViewReport == 'History Report') {
	$sGrossRevenueQuery = "SELECT sum((count(otDataHistory.email) * offers.revPerLead)) AS grossRevenue
	...
	$fAvgOffersTakenPerUser =  sprintf("%10.2f",$fAvgOffersTakenPerUser);
	$fAvgValidatedOffersTakenPerUser =  sprintf("%10.2f",$fAvgValidatedOffersTakenPerUser);
