...
if (hasAccessRight($iMenuId) || isAdmin()) {
	...
	if (($sViewReport)&& checkdate($iMonthFrom, $iDayFrom, $iYearFrom) && checkdate($iMonthTo, $iDayTo, $iYearTo)) {
		...
		if ($sAllowReport == 'N') {
			...
			if ($iActivePagesOnly == '1') {
				$bActivePagesOnly = true;
				$sNote2 = "Inactive Pages are excluded from this report.";
			} else {
				$bActivePagesOnly = false;
				$sNote2 = "Inactive Pages are included on this report.";
			}
			
			

			if ($aOfferCode[0] == 'all') {
				$sGetOfferCode = "SELECT distinct offerCode
							  		  FROM   offerStatsSum
							  		  WHERE  displayDate BETWEEN '$sDateFrom' AND '$sDateTo'";

				$rGetOfferCodeResult = dbQuery($sGetOfferCode);
				$iCount = 0;
				while ($oOfferCodeRow = dbFetchObject($rGetOfferCodeResult)) {
					$aOffers['offerCode'][$iCount] = $oOfferCodeRow->offerCode;
					$iCount++;
				}
			}
			...
			if ($sShowTop10 == 'byRev') {
				...
				if ($bActivePagesOnly == false) {
					...
					if ($aPageId[0] != 'all') {
						for ($i=0;$i<count($aPageId);$i++) {
							$iTempPgId = $aPageId[$i];
							$sCheckOtDataHistoryQuery = "SELECT count(*) as count FROM activePages WHERE pageId='$iTempPgId'";
							...
							if ($oActivePageRow->count > 0) {
								$sPagesSelected .= "'".$iTempPgId."',";
							} else {
								$sCheckOtPagesDateAddedQuery = "SELECT count(*) as count FROM otPages WHERE id = '$iTempPgId'
						...
						$sGetAllPageId = "SELECT * FROM otPages";
						$rGetAllPageId = dbQuery($sGetAllPageId);
						while ($oPageIdRow = dbFetchObject($rGetAllPageId)) {
							$sCheckOtDataHistoryQuery = "SELECT count(*) as count FROM activePages WHERE pageId='$oPageIdRow->id'";
							$rCheckOtDataHistoryResult = dbQuery($sCheckOtDataHistoryQuery);
							$oActivePageRow = dbFetchObject($rCheckOtDataHistoryResult);
							if ($oActivePageRow->count == 0) {
								$sCheckOtPagesDateAddedQuery = "SELECT count(*) as count FROM otPages WHERE id = '$oPageIdRow->id'
										AND date_format(dateTimeAdded, '%Y-%m-%d') BETWEEN date_add(CURRENT_DATE, INTERVAL -30 DAY)
										AND date_add(CURRENT_DATE, INTERVAL -0 DAY)";
								$rCheckOtPagesDateAddedResult = dbQuery($sCheckOtPagesDateAddedQuery);
								$oPageDateRow = dbFetchObject($rCheckOtPagesDateAddedResult);
								if ($oPageDateRow->count > 0) {
									$sPagesSelected .= "'".$oPageIdRow->id."',";
								}
							} else {
								$sPagesSelected .= "'".$oPageIdRow->id."',";
							}
			...
			if ($aOfferCode[0] != 'all') {
				...
				$sOffersQuery .= " ORDER BY offerCode";
					...
					for ($i=0; $i<count($aOffers['offerCode']); $i++) {
						$sOffersSelected .= "'".$aOffers['offerCode'][$i]."',";
					}
					if ($sOffersSelected !='') {
						$sOffersSelected = substr($sOffersSelected,0,strlen($sOffersSelected)-1);
						$sOffersQuery .= " AND offerCode IN (".$sOffersSelected.")";
			...
			$sAddQuery = "INSERT INTO nibbles.trackNibbleUse(userName, pageName, dateTimeLogged, action) 
			  VALUES('$sTrackingUser', '$PHP_SELF', now(), \"View report: Date range BETWEEN '$sDateFrom' AND '$sDateTo'.  SQL: $sOffersQuery\")"; 
	...
	if ($sViewReport == 'Export Report') {
		...
		$sFileName = "ecpm_".$iCurrMonth.$iCurrDay."_".$iCurrHH.$iCurrMM.$iCurrSS.".xls";
