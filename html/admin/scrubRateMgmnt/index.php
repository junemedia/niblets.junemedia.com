...
if (hasAccessRight($iMenuId) || isAdmin()) {
	...
	if ($sDelete) {
		$sDeleteQuery = "DELETE FROM offerScrubHistory WHERE  id = $iId";
	...
	if (!($sCurrOrder)) {
		switch ($sOrderColumn) {
			...
			break;
			case "scrubRate" :
			$sCurrOrder = $fScrubRateOrder;
			$fScrubRateOrder = ($fScrubRateOrder != "DESC" ? "DESC" : "ASC");
			break;
			case "dateTimeAdded" :
	...
	if ($sViewReport != '') {
		if (checkdate($iMonthFrom, $iDayFrom, $iYearFrom) && checkdate($iMonthTo, $iDayTo, $iYearTo) && !$bDateRangeNotOk) {
			...
			$sSelectQuery = "SELECT * FROM offerScrubHistory 
				...
				if (dbNumRows($rSelectResult) > 0) {
					...
					while ($oRow = dbFetchObject($rSelectResult)) {
						...
						$sList .= "<tr class=$sBgcolorClass><td>$oRow->offerCode</td>
										<td>$oRow->dateTimeAdded</td>
										<td>$oRow->scrubRate</td>
										<td><a href='JavaScript:void(window.open(\"add.php?iMenuId=$iMenuId&id=".$oRow->id."\", \"AddContent\", \"height=300, width=700, scrollbars=yes, resizable=yes, status=yes\"));'>Edit</a>
...
<td class=header><a href='<?php echo $sSortLink;?>&sOrderColumn=dateTimeAdded&sDateTimeAddedOrder=<?php echo $sDateTimeAddedOrder;?>' class=header>Date/Time</a></td>
<td class=header><a href='<?php echo $sSortLink;?>&sOrderColumn=scrubRate&fScrubRateOrder=<?php echo $fScrubRateOrder;?>' class=header>Scrub Rate</a></td>
</tr>
<?php echo $sList;?>
<tr><td colspan="3"><br><br>Notes:<br>
Approximate time to run this report - <?php echo $iScriptExecutionTime;?> second(s).<br>
<br>Query: <?php echo $sSelectQuery; ?><br>
</td>
</tr>
</table>
</form>
