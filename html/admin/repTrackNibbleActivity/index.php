...
if (hasAccessRight($iMenuId) || isAdmin()) {
		...
		if (checkdate($iMonthFrom, $iDayFrom, $iYearFrom) && checkdate($iMonthTo, $iDayTo, $iYearTo) && !$bDateRangeNotOk) {
			if ($sAllowReport == 'N') {
				$sMessage .= "<br>Server Load Is High. Please check back soon...";
					...
					$sReportQuery = "SELECT userName, pageName, dateTimeLogged
							FROM trackNibbleUse
				...
				if ($sUserName != "") {
					$sReportQuery .= " and userName = '$sUserName' ";
				}
				
				if ($sPageName != "") {
					$sReportQuery .= " and pageName = '$sPageName' ";
				}
				
				
				
				//$sReportQuery .= " GROUP BY messageId, listName";
				...
				$rReportResult = dbQuery($sReportQuery);
					...
					while ($oReportRow = dbFetchObject($rReportResult)) {
						...
						$sReportContent .= "<tr class=$sBgcolorClass>
								...
								<td>$oReportRow->pageName</td>
								<td>$oReportRow->userName</td></tr>";
						
						$sExpReportContent .= "$oReportRow->dateTimeLogged\t$oReportRow->pageName\t$oReportRow->userName\n";
					}
			}
		} else {
	...
	if ($sExportExcel && !$bDateRangeNotOk) {
		$sExpReportContent = "Date\tPage Name\tUser Name\n".$sExpReportContent;
	...
	while ($oRepRow = dbFetchObject($rRepResult)) {
		...
		// Get all page names
	$sPageNameQuery = "SELECT distinct pageName
				FROM trackNibbleUse
      			WHERE dateTimeLogged BETWEEN '$sDateFrom 00:00:00' AND '$sDateTo 23:59:59'
      			ORDER BY pageName ASC";

	$rPageNameResult = dbQuery($sPageNameQuery);
	echo dbError();

	while ($oPageNameRow = dbFetchObject($rPageNameResult)) {
		if ($sPageName == $oPageNameRow->pageName) {
			$sPageSelected = "selected";
		} else {
			$sPageSelected = "";
		}
		$sPageNameOption .= "<option value='".$oPageNameRow->pageName."' $sPageSelected>$oPageNameRow->pageName";
	}
