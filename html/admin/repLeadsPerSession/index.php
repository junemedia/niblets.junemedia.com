...
$sPageTitle = "Leads Per Session Report";
...
if (hasAccessRight($iMenuId) || isAdmin()) {
	...
	if (!($sCurrOrder)) {
		switch ($sOrderColumn) {
			...
			break;
			case "sessionId" :
			$sCurrOrder = $sSessionIdOrder;
			$sSessionIdOrder = ($sSessionIdOrder != "DESC" ? "DESC" : "ASC");
			break;
			case "subSourceCode" :
	...
	$sSortLink = $PHP_SELF."?iMenuId=$iMenuId&iYearFrom=$iYearFrom&iMonthFrom=$iMonthFrom&iDayFrom=$iDayFrom&iYearTo=$iYearTo&iMonthTo=$iMonthTo&iDayTo=$iDayTo
							&iDbMailId=$iDbMailId&sViewReport=$sViewReport&iRecPerPage=$iRecPerPage&sUserName=$sUserName&sPageName=$sPageName&iLeadsPerSession=$iLeadsPerSession&sSourceCode=$sSourceCode
	...
	if ($sViewReport != "") {
		if (checkdate($iMonthFrom, $iDayFrom, $iYearFrom) && checkdate($iMonthTo, $iDayTo, $iYearTo) && !$bDateRangeNotOk) {
			if ($sAllowReport == 'N') {
				...
				$sTempDelete = "TRUNCATE TABLE tempSessionStats";
				...
				if ($sShowSubSourceCode) {
					$sGroupByFilter = "GROUP BY sessionId, sourceCode, subSourceCode";
				} else {
					$sGroupByFilter = "GROUP BY sessionId";
				}

				$sTempQuery = "INSERT INTO tempSessionStats (sessionId,count,sourceCode,subSourceCode)
						SELECT sessionId, count(sessionId) as count, sourceCode, subSourceCode
						FROM otDataHistory
				...
				$sDelete = "DELETE FROM tempSessionStats WHERE count < $iLeadsPerSession";
				$rDeleteResult = dbQuery($sDelete);
				echo dbError();
				
				
				$sReportQuery = "SELECT * FROM tempSessionStats ORDER BY $sOrderColumn $sCurrOrder";
				...
				$rReportResult = dbQuery($sReportQuery);
					...
					while ($oReportRow = dbFetchObject($rReportResult)) {
							...
							if ($sShowSubSourceCode) {
								if ($oReportRow->sessionId == $sSesId) {
									$sReportContent .= "<tr class=$sBgcolorClass>
										<td><a href='$sSortLink&sSesId=$oReportRow->sessionId'>$oReportRow->sessionId</a></td>
...
<tr><td>Minimum Leads Per Session Id:</td><td>
<input type=text name=iLeadsPerSession value='<?php echo $iLeadsPerSession;?>' size=2>
</td></tr>
...
<table cellpadding=0 cellspacing=0 bgcolor=c9c9c9 width=80% align=center border=1 bordercolor=#000000>
	...
	<tr><?php echo $sDateSentHeader;?>
		<td class=header><a href="<?php echo $sSortLink;?>&sOrderColumn=sessionId&sSessionIdOrder=<?php echo $sSessionIdOrder;?>" class=header>Session Id</a></td>
...
<tr><td colspan=4 align=left><hr color=#000000></td></tr>	
	...
	<tr><td colspan=4>
		...
		Clicking on actual session id link will display related otData/userData.<br>
