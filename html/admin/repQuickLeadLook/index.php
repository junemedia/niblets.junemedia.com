...
if (hasAccessRight($iMenuId) || isAdmin()) {
	...
	if ( $sDelete ) {
		if (hasAccessRight('62') || isAdmin()) {
			...
			$sAddQuery = "INSERT INTO nibbles.trackNibbleUse(userName, pageName, dateTimeLogged, action) 
			  VALUES('$sTrackingUser', '$PHP_SELF', now(), \"Unauthorized attempt to delete lead: id = $iId\")"; 
			$rResult = dbQuery($sAddQuery); 
			echo  dbError(); 
			mysql_connect ($reportingHost, $reportingUser, $reportingPass); 
			mysql_select_db ($reportingDbase); 
		...
		if ($sAllowReport == 'N') {
			...
			if ($sFilter != '') {
				...
				switch ($sSearchIn) {
					...
					/*$sFilterPart .= ($iExactMatch) ? "o.offerCode = '$sFilter' || sourceCode = '$sFilter' || repDesignated = '$sFilter' || o.email = '$sFilter' || first = '$sFilter' ||
							...
							|| zip like '%$sFilter%'";*/
					$sFilterPart .= " 1=1 ";
					$sMessage .= "Please choose a Search Field.";
				}
			...
			if ($sViewReport != 'History Report') {
				...
				$sLeadQuery = "SELECT u.salutation, u.first, u.last, u.address, u.address2, u.city, u.state, u.zip, phoneNo,
									...
									AND   o.dateTimeAdded BETWEEN '$sDateTimeFrom' AND '$sDateTimeTo'
								    $sFilterPart ";
			...
			$sAddQuery = "INSERT INTO nibbles.trackNibbleUse(userName, pageName, dateTimeLogged, action) 
			  VALUES('$sTrackingUser', '$PHP_SELF', now(), \"View report: $sLeadQuery\")"; 
			$rResult = dbQuery($sAddQuery); 
			echo  dbError(); 
			mysql_connect ($reportingHost, $reportingUser, $reportingPass); 
			mysql_select_db ($reportingDbase); 
	...
	$sSearchInOptions = "<option value='' $sAllFieldsSelected>Choose One:
						...
						<option value='phoneNo' $sPhoneNoSelected>PhoneNo
						";
	...
	*/
	...
	while ($oRepRow = dbFetchObject($rRepResult)) {
		$sRepList .= " $oRepRow->id - $oRepRow->userName, ";
	}
