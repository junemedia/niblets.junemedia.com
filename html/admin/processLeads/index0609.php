...
if ($sExportData || $sImportData || $sProcessLeads || $sSendLeads || $sSendFormPostLeads) {
	...
	if ($sTestMode && $sTestProcessingEmailRecipients == '') {
		...
		/*************************  Comma separated data exported to .txt file for postal verification  ******************/
			...
			/********************  End of exporting data  *******************/
			
			
		} else if ($sImportData) {
			
			/******************  Import postao verified data  ********************/
			if ($_FILES['fImportFile']['tmp_name'] && $_FILES['fImportFile']['tmp_name']!="none") {
				...
				/*************  End getting uploaded file  ***********/
				
			
				/***********  Import data from postal verified file into temporary table  ***********/
				...
				/*************  End importing data into temporary table  *************/
				...
				echo dbError();
				/*********  End marking	Multimatched leads as valid leads  *********/
				...
				if ($rImportResult) {
					...
					/**********  Update userDataHistory and otDataHistory tables  **********/
					...
					while ($oTempDataRow = dbFetchObject($rTempDataResult)) {
						...
						$sOtDataUpdateQuery = "UPDATE otDataHistory
							...
							$sOtDataUpdateQuery = "UPDATE otDataHistory
								 SET    postalVerified = '$sTempPostalVerified'
								 WHERE  email = '$sTempEmail'
								 AND    (postalVerified IS NULL OR postalVerified = '')";
					...
					/***********  End updating userDataHistory and otDataHistory tables *********/	
					...
					echo dbError();
					/*************  End deleting records from temp table  *********/
					...
					/************** Mark remaining postalverified column of otDataHistory table
					 in case of user was existing before and postal verified/not verified  **********/
					
					echo"************** Mark remaining postalverified column of otDataHistory table
					 in case of user was existing before and postal verified/not verified  **********";

					$sOfferQuery = "SELECT *
									FROM   otDataHistory
									WHERE  (postalVerified IS NULL || postalVerified = '')
									ORDER BY id DESC";
					$rOfferResult = dbQuery($sOfferQuery);
					echo dbError();
					
					while ($oOfferRow = dbFetchObject($rOfferResult)) {		
						$sEmail = $oOfferRow->email;
						$sPostalVerified = '';
						$sTempQuery = "SELECT * 
									   FROM userDataHistory 
									   WHERE  email = '$sEmail'";
						...
						while ($oTempRow = dbFetchObject($rTempResult)) {
							$sPostalVerified = $oTempRow->postalVerified;
						}
		
						if ($sPostalVerified != '') {
							$sUpdateQuery = "UPDATE otDataHistory
											 SET    postalVerified = '$sPostalVerified'
											 WHERE  email = '$sEmail'";
							$rUpdateResult = dbQuery($sUpdateQuery);
							echo dbError();
						}
					...
					/**********  End marking remaining postalverified column *********/
					echo"**********  End marking remaining postalverified column *********"	;				
				} else {
			...
			echo"**************  get one offer/group which and make sure it is scheduled to process leads today  *****************";
			...
			echo"**************  End getting one offer/group and check it's schedule  ***********";
			...
			if ($sProcessOption == "rerun" || $sProcessOption == "rerunOne") {
				...
				echo"**************  get all active offers and process it's leads  **************";
			...
			echo"get the offers list/one offer to get leads for";
			if ($sOffersQuery != '') {
				...
				while ($oOffersRow = dbFetchObject($rOffersResult)) {
					...
					echo"***********  Replace tags with values in lead file name  **************";
					...
					echo"**********  Replace tags with values in headerText  ***********";
					if ($sTempHeaderText != '') {
					...
					echo"********** before getting new group data,
					 set header and footer in group lead file for previous groupId, only if file is combined   *************";
					...
					ECHO"***********  get lead specific data from leadGroups table if offer is grouped 
						 and lead group is not the same as previous loop  **************";
					...
					ECHO"************ get leads data for this offer										
					 get id of the lead record and mark it processed, one by one until mysql upgraded  **********";
					...
					if (! $rTempLeadsResult) {
						...
						echo"**************  Get the next lead count numbers for the offer  *************";
						if (!($sProcessOption == "rerunOne" || $sProcessOption == "rerunAll") ){
						...
						while ($aTempLeadsRow = dbFetchArray($rTempLeadsResult)) {
							...
							echo"********  Update process status and leadcounter only if lead not delivered real time  *******";
							...
							echo"*********  If delivery method is not 'leads in email body',
							  Prepare lead data as per specification details like field separator, field delimiter etc  *******";
							...
							echo"******** if delivery method = 13 - daily batch email - leads in email body
								replace fields with values in email body  ********";
							...
							echo"get all the page2 fields of this offer and replace ";
						...
						echo"***********  add header and footer text if file not grouped  **********";
						...
						echo"**************  Stored the prepared lead file  **************";
						...
						echo"create file and  store data in the file only if lead count is not 0";
						...
						echo"********** if offer is grouped, put the separate lead file of this offer in groups folder
						 or append to group file if it should be combined in one file  **********";
						...
						if ($iTempLeadsGroupId) {
							...
							echo"copy data into group file if have to combine";
		...
		echo"If the last record was with groupId or leads processed for only one group";
	...
	// get the offer list/ one offer to get leads for
		...
		echo"get the offer list/ one offer to get leads for";		
	...
	if ($sOffersQuery != '') {
		...
		while ($oOffersRow = dbFetchObject($rOffersResult)) {
			...
			echo"********  Get delivery method short description  ***********";
			...
			echo"************  Replace tags with values in header text  **************";
			...
			echo"************  Replace tags with values in leads email subject  ************";
			...
			echo"************  Replace tags with values in leads email body  ***********";
			...
			echo"*************  Replace tags with values in single email subject  *************";
			...
			/********  Get new group data,  send the leads of group  ********/	

			echo"********  Get new group data,  send the leads of group  ********";
			...
			if (($iTempLeadsGroupId != 0  && $iTempPrevGroupId != $iTempLeadsGroupId )) {
				...
				echo"**********  Replace tags with values in group lead file name  ***********";
				...
				echo"***********  Replace tags with values in group lead email subject  *********";
				...
				echo"******** get offercode wise lead counts and file names here *************";
				...
				echo"******** if testing of lead delivery, then use the email address specified in leads processing screen *********";
				...
				/***********  End if testing of lead delivery  **************/
						...
						echo"******  Update group leads and set sendStatus = 'S' for all the leads of the group  *****";
				...
				echo"If offer is not grouped and not processed in test mode";
				...
				echo"**********  Get last id reported for real time offers  **********";
				...
				echo"*******  Check if offer has any mutually exclusive offers  *********";
				...
				if (! $rTempLeadsResult) {
					...
					echo"********  update offerCount for this offer  **********";
					...
					while ($aTempLeadsRow = dbFetchArray($rTempLeadsResult)) {
						...
						echo"*******  Send leads one by one for form post or single email delivery method  ******";
						...
						echo"*********  Mark the leads as send which are not grouped  *********";
					...
					echo"***** insert lead counts and lastIdReported if leads were sent real time  ******";
					...
					echo"*********  Place lead counts in lead email sub, body and file name  **********";
					...
					echo"*********  Send the leads as per lead delivery method if offer not grouped **********";
...
// get lead counts of all the offers except real time and form post
	...
	//mail("smita@myfree.com", $sLeadsCountEmailSubject, $sCountsEmailContent, $sHeaders);
