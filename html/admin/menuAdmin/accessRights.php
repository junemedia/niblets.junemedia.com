...
$sPageTitle = "Nibbles Access Rights";
...
if (hasAccessRight($iMenuId) || isAdmin()) {
	...
	if ($iAccessRightMenuId && $sSubmit) {
		...
		while ($oUserRow = dbFetchObject($rUserResult)) {
			...
			$sTempAccessRight = $aAccessRight[$iTempUserId];			
						
			// get the permissions for this menu item, 
			// Update if exists, insert if not exist..
			$sCheckQuery = "SELECT *
						   FROM   accessRights
						   WHERE  menuId = $iAccessRightMenuId
						   AND    userId = $iTempUserId";
			...
			if (dbNumRows($rCheckResult)>0) {
				...
				$sInsertQuery = "INSERT INTO accessRights(menuId, userId, accessRight)
								VALUES('$iAccessRightMenuId', '$iTempUserId', '$sTempAccessRight')";

				// start of track users' activity in nibbles 
				$sTrackingUser = $_SERVER['PHP_AUTH_USER']; 
		
				$sAddLogQuery = "INSERT INTO nibbles.trackNibbleUse(userName, pageName, dateTimeLogged, action) 
				  VALUES('$sTrackingUser', '$PHP_SELF', now(), \"Update: $sInsertQuery\")"; 
				$rLogResult = dbQuery($sAddLogQuery); 
				echo  dbError(); 
				// end of track users' activity in nibbles		
	...
	if ($iAccessRightMenuId) {
		...
		$sUserQuery = "SELECT id, firstName, lastName
				  FROM   nbUsers
		...
		while ($oUserRow = dbFetchObject($rUserResult)) {
			...
			$sAccessRightsList .="<tr class=$sBgcolorClass><td>$oUserRow->firstName $oUserRow->lastName</td>";
			...
			$sAccessRightsList .= "<td><input type=checkbox name=aAccessRight[".$iUserId ."] value='Y' $sAccessRightChecked></td></tr>";
		}
	}
