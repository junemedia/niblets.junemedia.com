...
if ($sMessage == '') {
	...
	for ($i = 0; $i < count($_SESSION["aSesPage2Offers"]); $i++) {
		...
		if ($bDropped != "true") {
			...
			$sLeadInsertQuery = "INSERT INTO otData(email, offerCode, sourceCode, subSourceCode, pageId, dateTimeAdded, remoteIp, page2Data, mode )
						 VALUES(\"".$_SESSION["sSesEmail"]."\", \"$sOfferCode\", \"".$_SESSION["sSesSourceCode"]."\",  \"".$_SESSION["sSesSubSourceCode"].
			"\", \"".$_SESSION["iSesPageId"]."\", NOW(), '".$_SESSION["sSesRemoteIp"]."', \"$sPage2Data\", '".$_SESSION["sSesPageMode"]."')";
