...
$_SESSION["sSesServerIp"] = $sServerIp;
$_SESSION["iSesJoinListId"] = $iJoinListId;

if ($sSourceCode) {
	
	if (!isAlphaNumeric($sSourceCode)) {
		$sSourceCode = '';
	}
...
if ($sSubSourceCode) {
	...
	if (!isAlphaNumeric($sSubSourceCode)) {
		$sSubSourceCode = '';
	}
...
if (($sMessage == '' && count($aOffersChecked) > 0 ) || ($iOfferNotRequired && $sMessage == '')) {
	...
	for ($i = 0; $i < count($aOffersChecked); $i++) {
		...
		if ($iPage2Info) {
			...
			$sLeadInsertQuery = "INSERT IGNORE INTO otData(email, offerCode, revPerLead, sourceCode, subSourceCode, pageId, dateTimeAdded, remoteIp, serverIp, mode, sessionId )
								 VALUES(\"$sEmail\", \"".$aOffersChecked[$i]."\", \"$fRevPerLead\", \"$sSourceCode\", \"$sSubSourceCode\", \"$iPageId\", NOW(), '$sRemoteIp', '$sServerIp', '$sPageMode', '".session_id()."')";
