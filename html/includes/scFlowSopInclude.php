...
if (count($_SESSION['aSesFlowUrl']) > 0) {
	$sCurrentPage = "http://".$_SERVER['SERVER_NAME'].$_SERVER['PHP_SELF']."?".SID;

	$sCurrentPage = trim($_GET['oc']);
	
	// SC FLOW HEADER AND FOOTER
	$sScFlowHeader = $_SESSION['aScFlowHeader'][0];
	$sScFlowFooter = $_SESSION['sScFlowFooter'];

	//print_r($_SESSION['aSesFlowUrl']);


	$sRedirectTo = $_SESSION['aSesFlowUrl'][0];
	//echo $sRedirectTo;
	$sRedirectTo = strstr($sRedirectTo,'?oc=');
	
	$sGetTmPost = strpos($sRedirectTo,'&');
	$sCodeTo = substr($sRedirectTo,4,$sGetTmPost-4);
	if ($sCurrentPage == $sCodeTo) {
		unset($_SESSION['aSesFlowUrl'][0]);
		$temp_array = array_values($_SESSION['aSesFlowUrl']);
		$_SESSION['aSesFlowUrl'] = $temp_array;
		
		if (count($_SESSION['aScFlowHeader'])>1) {
			unset($_SESSION['aScFlowHeader'][0]);
			$stemp_array = array_values($_SESSION['aScFlowHeader']);
			$_SESSION['aScFlowHeader'] = $stemp_array;
		}
	}
	
		
	if (strstr($_SESSION['aSesFlowUrl'][0],"?")) {
		$sRedirectTo = $_SESSION['aSesFlowUrl'][0]."&".SID;
	} else {
		$sRedirectTo = $_SESSION['aSesFlowUrl'][0]."?".SID;
	}

	//var_dump($_SESSION['aSesFlowUrl']);
	// remove before copy to production.  only for test purpose
	unset($_SESSION['aScFlowHeaderTemp'][0]);
	unset($_SESSION['aScFlowHeaderTemp'][1]);
	$stemp_array = array_values($_SESSION['aScFlowHeaderTemp']);
	$_SESSION['aScFlowHeaderTemp'] = $stemp_array;
}
...
<table align='center' border='0' cellpadding='2' cellspacing='0'>
	...
	<tr>
		<td align='right' nowrap='nowrap'><font size='2' face='Arial,Helvetica'><b>Birth Date </b></font>
		</td>
	    ...
	    <font size='2' face='Arial,Helvetica'><b>Gender </b></font></td><td>
	    	<input type='radio' name='sGender' value='M' $sGenderDisable $sMaleChecked >Male&nbsp;&nbsp;&nbsp;
			&nbsp;&nbsp;<input type='radio' name='sGender' value='F' $sGenderDisable $sFemaleChecked >Female
			<span class='required'>required</span>
		</td></tr>
