...
if ($_SESSION['sSesTargetZip'] != '') { $sTargetZip = $_SESSION['sSesTargetZip']; }
if ($_SESSION['sSesTargetYear'] != '') { $iTargetYear = $_SESSION['sSesTargetYear']; }
if ($_SESSION['sSesTargetExchange'] != '') { $iTargetExchange = $_SESSION['sSesTargetExchange']; }
if ($_SESSION['sSesTargetGender'] != '') { $sTargetGender = $_SESSION['sSesTargetGender']; }
if ($_SESSION['sSesTargetState'] != '') { $sTargetState = $_SESSION['sSesTargetState']; }
...
$sOtPageQuery = "SELECT * FROM   otPages WHERE  id = '".$_SESSION["iSesPageId"]."'";
...
if ($_SESSION['iSesCbId'] != '') {
	$sGetCbColorsQuery = "SELECT * FROM   coBrandDetails WHERE  id = '".$_SESSION["iSesCbId"]."'";
...
$sOfferListLayoutQuery = "SELECT * FROM   offerListLayouts WHERE  id = '$iOfferListLayoutId'";
...
if ($iOffersByPageMap) {
	...
	if (substr($sPageName,0,4) == 'test') {
		...
		$sOffersQuery .= " AND    offers.mode = 'A'
	...
	*******************************************/
	...
	while ($oOffersRow = dbFetchObject($rOffersResult)) {
		...
		if ($bShowOffer == true) {
			...
			if ($oOffersRow->precheckAllPages) { $aOffersArray['precheck'][$i] = '1'; }
	...
	for ($o=0; $o < count($aOffersArray['offerCode']); $o++) {
		...
		if ($sIsCloseTheyHost == 'Y' && $sCloseTheyHostTriggerOn == 'N') {
			// If $sOfferCode is closeTheyHost and triggerOn is No
	...
	if ($sPageMode != 'T') {
		...
		if (isset($_SESSION['sSesNewSession']) && $_SESSION['sSesNewSession']=='Y') {
			$sPageStatQuery = "INSERT INTO tempPageDisplayStats(pageId, sourceCode, subSourceCode, openDate, sessionId, ipAddress, openDateTime)
							   VALUES('$iPageId', '$sSourceCode', '$sSubSourceCode',CURRENT_DATE,'".session_id()."', '".$_SERVER['REMOTE_ADDR']."', now())";
		...
		while ($oPixelCheckRow = dbFetchObject($rPixelCheckResult)) {
			...
			/************   End displaying incoming var in pixel code  ************/
