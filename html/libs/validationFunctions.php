...
function makeQuerySafeToRun ($IncomingVar) {
	$aUnSafeChars = array();
	if (get_magic_quotes_gpc()) {
		// If magic quotes is enabled - turn the string back into an unsafe string
		$IncomingVar = stripslashes($IncomingVar);
	}
	
	if (count($aUnSafeChars) > 0) {
		foreach ($aUnSafeChars as $temp) {
			$IncomingVar = str_replace($temp,'',$IncomingVar);
		}
	}
	
	// Now convert the unsafe string into a MySQL safe string
	$IncomingVar = mysql_real_escape_string($IncomingVar);
	return $IncomingVar;
	// $IncomingVar should now be safe to insert into a query
}
	...
	*/
	if ($valid == false) {
		$sErrorLogQuery = "INSERT INTO errorLog(errorDateTime, valueInvalidated, function, ipAddress, sourceCode, pageId )
						 VALUES(now(), '$name', '$sFunction', '".$_SESSION["sSesRemoteIp"]."',\"".$_SESSION["sSesSourceCode"]."\",\"".$_SESSION["iSesPageId"]."\")";
...
function validateAddress($address)
{
	...
	if ( eregi("[aeiouy]{4,}", $address) || eregi("[^aeiouy0-9[:space:]\.]{6,}", $address)) {
	...
	if ($valid == false) {
		//echo("banned word");
		$sErrorLogQuery = "INSERT INTO errorLog(errorDateTime, valueInvalidated, function, ipAddress, sourceCode, pageId)
						 VALUES(now(), '$address', '$sFunction', '".$_SESSION["sSesRemoteIp"]."',\"".$_SESSION["sSesSourceCode"]."\",\"".$_SESSION["iSesPageId"]."\")";
		$rErrorLogResult = dbQuery($sErrorLogQuery);
...
function validateEmailFormat($eMail) {
	...
	if (!($valid)) {
		$sErrorLogQuery = "INSERT INTO errorLog(errorDateTime, valueInvalidated, function, ipAddress, sourceCode, pageId)
						 VALUES(now(), '$eMail', '$sFunction', '".$_SESSION["sSesRemoteIp"]."',\"".$_SESSION["sSesSourceCode"]."\",\"".$_SESSION["iSesPageId"]."\")";
		$rErrorLogResult = dbQuery($sErrorLogQuery);
		echo dbError();
...
function isBannedDomain($email, &$sDomainName) {
	...
	if ($isBanned == true) {
		$sErrorLogQuery = "INSERT INTO errorLog(errorDateTime, valueInvalidated, function, ipAddress, sourceCode, pageId)
						 VALUES(now(), '$sDomainName', '$sFunction', '".$_SESSION["sSesRemoteIp"]."',\"".$_SESSION["sSesSourceCode"]."\",\"".$_SESSION["iSesPageId"]."\")";
		$rErrorLogResult = dbQuery($sErrorLogQuery);
...
function isBannedIp($ip) {
	...
	if ($isBanned == true) {
		$sErrorLogQuery = "INSERT INTO errorLog(errorDateTime, valueInvalidated, function, ipAddress, sourceCode, pageId)
						 VALUES(now(), '$ip', '$sfunction', '".$_SESSION["sSesRemoteIp"]."',\"".$_SESSION["sSesSourceCode"]."\", pageId)";
		$rErrorLogResult = dbQuery($sErrorLogQuery);
...
function validatePhone($area, $exchange, $four, $extension='', $state='',$sSourceCode='') {
	...
	//echo "$area, $exchange, $four, $extension, $state";
	...
	if ($valid == false) {
		$sErrorLogQuery = "INSERT INTO errorLog(errorDateTime, valueInvalidated, function, ipAddress, sourceCode, pageId)
						 VALUES(now(), \"$area, $exchange, $four, $extension, $state\", '$sFunction', '".$_SESSION["sSesRemoteIp"]."',\"".$_SESSION["sSesSourceCode"]."\",\"".$_SESSION["iSesPageId"]."\")";
		$rErrorLogResult = dbQuery($sErrorLogQuery);
...
function isNumberSeries($iNumber) {
	...
	if ($series == true) {
		$sErrorLogQuery = "INSERT INTO errorLog(errorDateTime, valueInvalidated, function, ipAddress, sourceCode, pageId)
							 VALUES(now(), '$iNumber', '$sFunction', '".$_SESSION["sSesRemoteIp"]."',\"".$_SESSION["sSesSourceCode"]."\",\"".$_SESSION["iSesPageId"]."\")";
		$rErrorLogResult = dbQuery($sErrorLogQuery);
	}
	return $series;
}
...
function httpFormPostGet ($sHttpPostString,$sUrlPart,$sPostingUrl,$iDeliveryMethodId,$sOfferCode,$sEmail,$sHowSent) {
	...
	/*
	if($sOfferCode == 'DSC_MES'){
		//mail('bbevis@amperemedia.com',__line__.' : validation things',  "$sHttpPostString,\n\n$sUrlPart,\n\n$sPostingUrl,\n\n$iDeliveryMethodId,\n\n$sOfferCode,\n\n$sEmail,\n\n$sHowSent, \n\n $rSocketConnection, \n\n$sHostPart");
	}*/
	if ($rSocketConnection) {
		if ($iDeliveryMethodId == '2') {	// http form post - GET
			...
			if($sOfferCode == 'R411_TAX'){
				fputs($rSocketConnection, "Referer: http://www.411taxrelief.com/index.cgi?cid=2270\r\n");
			}
		...
		if ($sOfferCode != 'VIDEOPROF') {
			if(strstr($sPostingUrl,'https://')) {
				while(!feof($rSocketConnection)) {
					//2006-12-21, BB -- I error suppressed the following line for a "fatal protocol error" that was showing up on SSL APIs.
