...
function validateName($name)
{
	...
	*/
	//|| eregi("[^aeiou]{5,}", $name)
	if ( eregi("[aeiou]{4,}", $name) || eregi("[^aeiou\.']{6,}", $name)) {
	...
	*/
	if ($valid == false) {
		$sErrorLogQuery = "INSERT INTO errorLog(errorDateTime, valueInvalidated, function, ipAddress, sourceCode )
						 VALUES(now(), '$name', '$sFunction', '".$_SESSION["sSesRemoteIp"]."',\"".$_SESSION["sSesSourceCode"]."\")";
...
function validateAddress($address)
{
	...
	if ( eregi("[aeiou]{4,}", $address) || eregi("[^aeiou0-9[:space:]\.]{6,}", $address)) {
	...
	if ($valid == false) {
		$sErrorLogQuery = "INSERT INTO errorLog(errorDateTime, valueInvalidated, function, ipAddress, sourceCode)
						 VALUES(now(), '$address', '$sFunction', '".$_SESSION["sSesRemoteIp"]."',\"".$_SESSION["sSesSourceCode"]."\")";
		$rErrorLogResult = dbQuery($sErrorLogQuery);
...
function validateCityStateZip($city, $state, $zipCode)
{
	...
	if ($valid == false) {
		$sErrorLogQuery = "INSERT INTO errorLog(errorDateTime, valueInvalidated, function, ipAddress, sourceCode)
						 VALUES(now(), '$city, $state, $zipCode', '$sFunction', '".$_SESSION["sSesRemoteIp"]."',\"".$_SESSION["sSesSourceCode"]."\")";
		$rErrorLogResult = dbQuery($sErrorLogQuery);
...
function isBannedDomain($email, &$sDomainName) {
	...
	if ($isBanned == true) {
		$sErrorLogQuery = "INSERT INTO errorLog(errorDateTime, valueInvalidated, function, ipAddress, sourceCode)
						 VALUES(now(), '$sDomainName', '$sFunction', '".$_SESSION["sSesRemoteIp"]."',\"".$_SESSION["sSesSourceCode"]."\")";
		$rErrorLogResult = dbQuery($sErrorLogQuery);
...
function isBannedIp($ip) {
	...
	if ($isBanned == true) {
		$sErrorLogQuery = "INSERT INTO errorLog(errorDateTime, valueInvalidated, function, ipAddress, sourceCode)
						 VALUES(now(), '$ip', '$sfunction', '".$_SESSION["sSesRemoteIp"]."',\"".$_SESSION["sSesSourceCode"]."\")";
		$rErrorLogResult = dbQuery($sErrorLogQuery);
...
function isBannedWord($aString) {
	...
	if ($isBadWord == true) {
		$sErrorLogQuery = "INSERT INTO errorLog(errorDateTime, valueInvalidated, function, ipAddress, sourceCode)
						 VALUES(now(), '$aString', '$sFunction', '".$_SESSION["sSesRemoteIp"]."',\"".$_SESSION["sSesSourceCode"]."\")";
		$rErrorLogResult = dbQuery($sErrorLogQuery);
...
function validate($sString, $sCheck) {
	...
	if ($bValid == false) {
		$sErrorLogQuery = "INSERT INTO errorLog(errorDateTime, valueInvalidated, function, ipAddress, sourceCode)
						 VALUES(now(), '$sString', '$sFunction', '".$_SESSION["sSesRemoteIp"]."',\"".$_SESSION["sSesSourceCode"]."\")";
		$rErrorLogResult = dbQuery($sErrorLogQuery);
...
function validatePhone($area, $exchange, $four, $extension='', $state='', $sSourceCode='') {
	...
	if ($valid == false) {
		$sErrorLogQuery = "INSERT INTO errorLog(errorDateTime, valueInvalidated, function, ipAddress, sourceCode)
						 VALUES(now(), \"$area, $exchange, $four, $extension, $state\", '$sFunction', '".$_SESSION["sSesRemoteIp"]."',\"".$_SESSION["sSesSourceCode"]."\")";
		$rErrorLogResult = dbQuery($sErrorLogQuery);
...
function exceedsMaxDistance($area1, $area2, $maxDistance, $sSourceCode='') {
	...
	$dist = getDistance($area1, $area2);
	
	if ( $dist > $maxDistance) {
				
		$sErrorLogQuery = "INSERT INTO errorLog(errorDateTime, valueInvalidated, function, ipAddress, sourceCode)
						 VALUES(now(), \"$area1, $area2\", '$sFunction', '".$_SESSION["sSesRemoteIp"]."',\"".$_SESSION["sSesSourceCode"]."\")";
		$rErrorLogResult = dbQuery($sErrorLogQuery);
		echo dbError();
		return false;	
...
function validateZip($sZipCode) {
	...
	if ($valid == false) {
		$sErrorLogQuery = "INSERT INTO errorLog(errorDateTime, valueInvalidated, function, ipAddress, sourceCode)
						 VALUES(now(), '$sZipCode', '$sFunction', '".$_SESSION["sSesRemoteIp"]."',\"".$_SESSION["sSesSourceCode"]."\")";
		$rErrorLogResult = dbQuery($sErrorLogQuery);
...
function isNumberSeries($iNumber) {
	...
	if ($series == true) {
		$sErrorLogQuery = "INSERT INTO errorLog(errorDateTime, valueInvalidated, function, ipAddress, sourceCode)
							 VALUES(now(), '$iNumber', '$sFunction', '".$_SESSION["sSesRemoteIp"]."',\"".$_SESSION["sSesSourceCode"]."\")";
		$rErrorLogResult = dbQuery($sErrorLogQuery);
	}
	return $series;
}
...
function validateCreditCard($sCardType, $iCardNumber)
{
	...
	if ( ( strtolower(substr($sCardType, 0, 4)) == "amex" ) && !(isAmericanExpress($iCardNumber))) {
		//alert('amex invalid');
		$bValid =  false;
	}
...
function validateData($aDataArray) {
	$j=0;
	$aResultArray = '';
	foreach ($aDataArray[0] AS $key => $sValidateAs) {
		
		switch($aDataArray[0][$key]) {
			
			case "name":
				// validate as name
				$bValid = validateName($aDataArray[1][$key]);				
				$aResultArray[$j][0] = "name";
				$aResultArray[$j][1] = $aDataArray[1][$key];
				$aResultArray[$j][2] = $bValid;
				$j++;
				break;
			case "address":
				// validate as address
				break;
			case "cityStateZip":
				// validate as city, state, zip. Incoming values are comma separated
				break;
			case "emailFormat":
				// validate using validateEmailFormat()
				break;
			case "validDomain":
				// validate using isValidDomain()
				break;
			case "bannedDomain":
				// validate using isBannedDomain()
				break;
			case "bannedIp":
				break;
			case "bannedWord":
