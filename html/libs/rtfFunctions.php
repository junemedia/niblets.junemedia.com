...
class rtf {
	...
	function rtf($config="rtfConfig.php") {
		require_once($config);
	...
	function getDocument() {
		...
		$File = $_SERVER['PHP_AUTH_USER'].date('Y').date('m').date('d').date('H').date('i').date('s');
		header("Content-Type: text/enriched\n");
		header("Content-Disposition: attachment; filename=$File.rtf");
		//header('Content-Length: '.strlen($this->buffer));
		//header("Content-disposition: inline; filename=$File.rtf");
		echo $this->buffer;
	}
